...
class JdbcInvestmentPlanGroupDaoIntegrationTest {
    ...
    private static JdbcInvestmentPlanGroupDao dao;
    ...
    static void setUp() {
        ...
        dao = new JdbcInvestmentPlanGroupDao(connectionFactory, configurationService);
    }
    ...
    @Test
    void findAllWithDisplayNameReturnsPlanGroups() {
        List<InvestmentPlanGroup> groups = dao.findAllWithDisplayName();
        assertNotNull(groups);
        assertTrue(groups.size() >= 0);
        if (!groups.isEmpty()) {
            InvestmentPlanGroup first = groups.get(0);
            assertNotNull(first.planGroupKey());
