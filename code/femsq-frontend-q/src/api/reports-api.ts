 ...
 *
 * ВАЖНО:
 * - Этот клиент **осознанно** использует REST (`/api/v1/reports/*`) и `fetch`;
 * - Операции генерации отчётов (`generateReport`, `generatePreview`) возвращают бинарные данные (`Blob`, PDF),
 *   которые невозможно передать через GraphQL (JSON-протокол);
 * - Согласно `.cursorrules` (`Правила архитектуры API`) бинарные отчётные эндпоинты являются допустимым
 *   исключением и не считаются нарушением запрета на доменный REST;
 * - Все остальные доменные API-клиенты должны использовать GraphQL/Apollo.
 */
