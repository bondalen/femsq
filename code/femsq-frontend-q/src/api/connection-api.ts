 ...
 *
 * ВАЖНО:
 * - Этот клиент **осознанно** использует REST (`/api/v1/connection/*`) через `apiGet`/`apiPost`;
 * - Эндпоинты подключения относятся к техническим `status/health`-операциям и не являются доменными;
 * - Таймаут первого запроса увеличен до 30 секунд для корректного health-check при загрузке приложения;
 * - Согласно `.cursorrules` (`Правила архитектуры API`) такой REST допускается и не считается нарушением,
 *   в то время как все доменные клиенты обязаны использовать GraphQL/Apollo.
 */
