...
**Архитектура:** Java-код (`ReportGenerationService`) не вызывает `spMstrg` напрямую.  
Он читает из предзаполненных таблиц `ags.spMstrg_2408_ResultSet1..7` через JRXML-шаблон.  
Таблицы заполняются внешним shell-скриптом **до** запуска генерации отчёта.

**Что меняется:**
- Скрипт заполнения `code/scripts/execute_spMstrg_2408.sh` заменяется на `code/scripts/execute_spMstrg_2605.sh`
- JRXML-шаблон `mstrgAg_23_Branch_q2m_2408_25.jrxml` **не меняется** (читает те же таблицы)
- Java-код `ReportGenerationService.java` **не меняется**

**Порядок обновления на сервере:**
1. Убедиться, что `ags.spMstrg_2605` создана (скрипт 03)
2. Заменить вызов на новый скрипт:
```bash
# БЫЛО:
...
```
3. Настроить параметры в `execute_spMstrg_2605.sh`:
   - `IPGCH`, `MONTH_END_DATE` — как раньше
   - `IPGST=""` — пустая строка = NULL = все стройки (текущий режим)

**Проверка результата:**
После выполнения скрипта убедиться в COUNT (для `ipgCh=15, MounthEndDate='2025-07-31'`):
```
RS1=12693, RS2=12693, RS3=12693, RS4=744, RS5=32, RS6=721, RS7=32
```

**Примечание:** `Записей сохранено: 0` в выводе процедуры — косметический артефакт  
обёртки IF/ELSE (@@ROWCOUNT захватывается не сразу после INSERT). На данные не влияет.
...
> Ожидаемые значения подтверждены на FishEye dev 2026-05-16 (ipgCh=15, MounthEndDate='2024-08-31')
...
| `fnIpgChRsltCstUtl2` столбцы: 2408 vs 2605 | **90 = 90** | | |
| `fnIpgChRsltCstUtlPercentBrn` столбцы: 2408 vs 2605 | **398 = 398** | | |
| `spMstrg_2605(ipgSt=NULL, save=1)` RS1 | **12 693** | | |
| `spMstrg_2605(ipgSt=NULL, save=1)` RS2 | **12 693** | | |
| `spMstrg_2605(ipgSt=NULL, save=1)` RS3 | **12 693** | | |
| `spMstrg_2605(ipgSt=NULL, save=1)` RS4 | **0** | | |
| `spMstrg_2605(ipgSt=NULL, save=1)` RS5 | **1** | | |
| `spMstrg_2605(ipgSt=NULL, save=1)` RS6 | **0** | | |
| `spMstrg_2605(ipgSt=NULL, save=1)` RS7 | **1** | | |
| `spMstrg_2605(ipgSt='12ОПР', save=1)` RS1 | **604** | | |
| `spMstrg_2605(ipgSt='12ОПР', save=1)` RS2 | **604** | | |
| `spMstrg_2605(ipgSt='12ОПР', save=1)` RS3 | **604** | | |

> **Примечание по ожидаемым значениям:** значения `RS1=12693` и `RS1=604` получены для  
> `ipgCh=15, MounthEndDate='2024-08-31'` на базе FishEye dev. На продуктиве при иных параметрах  
> значения будут отличаться — важно убедиться, что `RS_2605 = RS_2408_SaveToTables` при тех же параметрах.
...
| 1.1 | 2026-05-16 | Раздел 8.2: добавлены ожидаемые значения RS1..7 из тестового прогона на FishEye dev; добавлен 00_VERIFY_before |
| 1.2 | 2026-05-16 | Добавлен чеклист дня деплоя; в 04_VERIFY_after — секции 2c/2d (COUNT функций) |
