...
3. **Запустить приложение с внешними библиотеками**
   ...
   В Spring Boot 3.x `-Dloader.path` больше не используется для загрузки внешних библиотек. Вместо этого приложение запускается через classpath и Spring Boot JarLauncher.

   **Windows:**
   ```cmd
   java -cp "femsq-web-0.1.0.1-SNAPSHOT-thin.jar;lib\*.jar" org.springframework.boot.loader.launch.JarLauncher
   ```

   **Linux:**
   ```bash
   java -cp "femsq-web-0.1.0.1-SNAPSHOT-thin.jar:lib/*" org.springframework.boot.loader.launch.JarLauncher
   ```
...
```bash
java -Dfemsq.verify.lib.sha256=true -cp "femsq-web-*-thin.jar:lib/*" org.springframework.boot.loader.launch.JarLauncher
```
