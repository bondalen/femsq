...
#### Порядок доступа к шаре на nb-win (приоритет)
...
Linux-монтирование `/mnt/nb-win-share` нужно, чтобы **достать файл** с Fedora (CIFS) или выровнять WSL с Fedora (bind). Это не канон для колонок БД.

Исторические значения `ags.ra_dir.dir` / `af_name` в виде `/mnt/nb-win-share/...` остаются валидными POSIX: резолвер открывает их как есть. Новые поля (в т.ч. `cidufPath`) — конвенция Проводника.
