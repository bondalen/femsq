...
**Правило:** пути, которые попадают в БД (`ra_dir.dir`, `af_name`), всегда пишутся в конвенции `/mnt/nb-win-share/...` (переносимость на Fedora), даже если на nb-win для быстрой проверки файла удобнее заглянуть через `/mnt/d/wire-guard-share-nb-win/...`.
