...
**Автор:** Александр  
**Этап:** 02.1.3  
**Последнее обновление:** 2026-08-14 — master–detail как Access; вкладки идентификаторы / общее / разные имена (`ogs.ogNmF`); КПП в `org_id_value_t_ext`
...
**Создание / идентификаторы (2026-08-14):**
- Layout **master–detail**: список организаций **сверху**, детали **снизу** (как Access `og`).
- Вкладки детали:
  - **идентификаторы** — грид `ags.org_id` (org_id_key, тип БУИРГ/ИНН, цифровой/текстовый ключ, **расширение текстового = КПП** для филиалов);
  - **общее** — поля карточки `og` + контактные лица;
  - **разные имена для ловли** — грид `ags.ogNmF` (`onfName` / `onfNameExt` / даты актуальности).
- Кнопка **Создать** — диалог og + опционально БУиРГ / ИНН / КПП. ИНН+КПП пишутся в `org_id`, не в `og.ogINN`.
- Воронка СУДЗ по-прежнему матчит в основном по БУиРГ (type=1); КПП нужен для различения филиалов в учёте.
