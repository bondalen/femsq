-- 10_CREATE_vw_Yr_DbtFact_P1.sql
-- CstAgPnCode/Name: denorm ducaCode/ducaName при наличии строки DbtUplCstAg (P1 A/B/C).
-- AgOrg только при ducaCstAgPn IS NOT NULL.
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
SELECT
    ...
    CASE
        WHEN duca.ducaKey IS NOT NULL THEN duca.ducaCode
        ELSE pn.cstapIpgPnN
    END AS CstAgPnCode,
    CASE
        WHEN duca.ducaKey IS NOT NULL THEN duca.ducaName
        ELSE cst.cstName
    END AS CstAgPnName,
    CASE
        WHEN duca.ducaCstAgPn IS NOT NULL THEN agog.ogNm
        ELSE NULL
    END AS AgOrg
FROM sudz.yr AS y
...
PRINT N'10 vw_Yr_DbtFact P1 created';
GO
