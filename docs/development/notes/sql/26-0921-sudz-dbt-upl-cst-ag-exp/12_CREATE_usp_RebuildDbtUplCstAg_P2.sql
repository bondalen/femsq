-- P2 DEV: after A/B, promote Exp duexRule=13 into DbtUplCstAg, then C residual
-- Requires DbtUplCstAgExp for @dbtUpl with 1.3 rows (RebuildCstAgExp first)
-- Generated 2026-09-22 from 09_..._P1.sql
-- 09_CREATE_usp_RebuildDbtUplCstAg_P2.sql
...
BEGIN
    ...
    /* ---------- P2: 1.3 from Exp (year→all) → A/B before C ---------- */
    /* Unique/agent: Exp duexRule=13 with duexCstAgPn */
    INSERT INTO sudz.DbtUplCstAg (ducaDbt, ducaUpl, ducaCstAgPn, ducaCode, ducaName)
    SELECT e.duexDbt, @dbtUpl, e.duexCstAgPn,
           CAST(pn.cstapIpgPnN AS nvarchar(500)),
           CAST(ISNULL(cst.cstName, N'') AS nvarchar(500))
    FROM sudz.DbtUplCstAgExp AS e
    INNER JOIN ags.cstAgPn AS pn ON pn.cstapKey = e.duexCstAgPn
    LEFT JOIN ags.cstAg AS ca ON ca.cstaKey = pn.cstapCsta
    LEFT JOIN ags.cst AS cst ON cst.cstKey = ca.cstaCst
    WHERE e.duexUpl = @dbtUpl
      AND e.duexRule = 13
      AND e.duexCstAgPn IS NOT NULL
      AND NOT EXISTS (
            SELECT 1 FROM sudz.DbtUplCstAg d
            WHERE d.ducaUpl = @dbtUpl AND d.ducaDbt = e.duexDbt
      );

    /* Multi: Exp 1.3 multi without pn → B format */
    INSERT INTO sudz.DbtUplCstAg (ducaDbt, ducaUpl, ducaCstAgPn, ducaCode, ducaName)
    SELECT e.duexDbt, @dbtUpl, NULL,
           CAST(
             CASE WHEN LEFT(LTRIM(e.duexCode), 7) = N'кодов -'
                  THEN e.duexCode
                  ELSE N'кодов - ?: ' + ISNULL(e.duexCode, N'')
             END AS nvarchar(500)
           ),
    ...
    WHERE e.duexUpl = @dbtUpl
      AND e.duexRule = 13
      AND e.duexCstAgPn IS NULL
      AND e.duexName = N'1.3 multi'
      AND NOT EXISTS (
            SELECT 1 FROM sudz.DbtUplCstAg d
            WHERE d.ducaUpl = @dbtUpl AND d.ducaDbt = e.duexDbt
      );
