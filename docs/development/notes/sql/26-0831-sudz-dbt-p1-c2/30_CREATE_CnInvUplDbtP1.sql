...
-- Префикс cip1*; исчезновение Dbt base yr → curr + sum-match cidutDebt.
-- =============================================================================
...
BEGIN
    ...
    (
        ...
        cip1UnloadKey      int            NOT NULL,  -- curr upl_key
        cip1BaseUpl        int            NOT NULL,  -- yr.cn_inv_dbt_upl
        cip1DbtFile        int            NULL,      -- → sudz.CnInvDbtUplFile.cidufKey

        cip1DbtKey         int            NOT NULL,  -- исчезнувший канон Dbt
        cip1BaseSlotKey    int            NOT NULL,  -- invDbt.idKey на base
        ...
        cip1SumKind        varchar(8)     NOT NULL,  -- ttl | overd

        cip1CandCidut      int            NULL,      -- кандидат Tbl (NULL = нет совпадения)
        ...
        cip1Reason         varchar(16)    NOT NULL,  -- single | multi | none
...
WHERE s.name = N'sudz' AND t.name = N'CnInvUplDbtP1';
GO
