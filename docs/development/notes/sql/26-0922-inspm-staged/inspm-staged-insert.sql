/*
  InsPmNotLoad — staged build + INSERT в ags.cn_inv_pm.
  @uk = ciputUnloadKey. Совместимость: SQL Server 2012+.
  UAT 2026-09-22 @60: ready=270, insert=270, wall≈180 мс.
*/
SET NOCOUNT ON;
DECLARE @uk int = 60; /* параметр */
DECLARE @t0 datetime2, @wall datetime2 = SYSDATETIME(), @inserted int;
...
CREATE TABLE #prof (stage varchar(40) NOT NULL, n int NOT NULL, ms int NOT NULL);
...
INNER JOIN ags.invNum AS n ON n.inInv = ci.ciInv AND n.inNumNull = o.ciputCnInv
...
INNER JOIN ags.cnInvAccntSmpl AS f
  ON f.ciasCnInv = o.ciKey AND f.ciasCn_s_org_smpl = o.csosKey AND f.ciasAccnt = a.account_key
INNER JOIN #stg AS s
...
COMMIT TRAN;
INSERT INTO #prof VALUES ('08_insert', @inserted, DATEDIFF(ms, @t0, SYSDATETIME()));
INSERT INTO #prof VALUES (
  '99_wall',
  (SELECT COUNT(*) FROM ags.cn_inv_pm WHERE cn_inv_pm_upl = @uk),
  DATEDIFF(ms, @wall, SYSDATETIME())
);
