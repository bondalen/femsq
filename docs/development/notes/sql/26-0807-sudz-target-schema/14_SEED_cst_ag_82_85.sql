...
---------------------------------------------------------------------
IF OBJECT_ID(N'sudz.DbtUplCstAg', N'U') IS NULL
BEGIN
    CREATE TABLE sudz.DbtUplCstAg
    (
...
DELETE FROM sudz.DbtUplCstAg WHERE ducaDbt IN (82, 85);
...
DELETE FROM sudz.cnInvCmmCst WHERE ciccInvAccnt IN (82, 85);
DELETE FROM sudz.cnInvCmmAg  WHERE cicaInvAccnt IN (82, 85);
...
SELECT ducaDbt, ducaUpl, ducaCstAgPn FROM sudz.DbtUplCstAg ORDER BY 1, 2;
SELECT cnicInvAccnt, cnicType, LEFT(cnicText, 40) AS txt
FROM sudz.cnInvCmm WHERE cnicInvAccnt IN (82, 85) ORDER BY 1, 2;
GO
