-- =============================================================================
-- sudz.DbtValue — величина долга в выгрузке (песочница СУДЗ)
-- DEV only. Спецификация: 08-target-schema.md §2.5 / §4.1
-- =============================================================================

IF OBJECT_ID(N'sudz.DbtValue', N'U') IS NULL
BEGIN
    CREATE TABLE sudz.DbtValue
    (
...
IF NOT EXISTS (
    ...
    WHERE ep.major_id = OBJECT_ID(N'sudz.DbtValue')
      AND ep.minor_id = 0
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
        FROM inserted i
        INNER JOIN sudz.invDbtVar v ON v.idvvKey = i.dvInvDbtVar
    ...
    IF EXISTS (
        ...
        INNER JOIN sudz.invDbtVar v ON v.idvvKey = i.dvInvDbtVar
        INNER JOIN ags.invNum n ON n.inKey = v.idvvInvNum
        INNER JOIN sudz.invDbtDbt b
            ON b.iddInv = n.inInv
           AND b.iddDbt = i.dvDbt
        WHERE NOT EXISTS (
            SELECT 1
            FROM sudz.invDbtDbtVar bv
...
WHERE tr.parent_id = OBJECT_ID(N'sudz.DbtValue');
GO
