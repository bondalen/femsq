...
CREATE OR ALTER PROCEDURE sudz.Yr_DbtChangesD644Svod
    @yr       int,
    @curr_upl int = NULL
AS
BEGIN
    ...
    BEGIN
        RAISERROR(N'sudz.Yr_DbtChangesD644Svod: yr_key=%d не найден', 16, 1, @yr);
        RETURN;
    END;
    ...
    IF @curr_upl IS NOT NULL
        SELECT @curr_date = upl_date FROM sudz.cn_inv_dbt_upl WHERE upl_key = @curr_upl;
    ELSE
    BEGIN
    ...
    BEGIN
        RAISERROR(N'sudz.Yr_DbtChangesD644Svod: нет base/curr дат для yr=%d', 16, 1, @yr);
        RETURN;
    END;
