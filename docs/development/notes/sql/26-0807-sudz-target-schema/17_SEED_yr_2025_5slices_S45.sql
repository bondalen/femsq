...
USING (VALUES
    (801, CAST('2025-01-24' AS datetime), CAST('2024-12-31' AS date), N'[sudz] YE2024 / Rslt_26-0212 base'),
    (802, CAST('2025-04-21' AS datetime), CAST('2025-03-31' AS date), N'[sudz] Q1 2025 / Rslt_26-0212'),
    (803, CAST('2025-07-18' AS datetime), CAST('2025-06-30' AS date), N'[sudz] Q2 2025 / Rslt_26-0212'),
    (804, CAST('2025-10-21' AS datetime), CAST('2025-09-30' AS date), N'[sudz] Q3 2025 / Rslt_26-0212'),
    (805, CAST('2026-01-30' AS datetime), CAST('2025-12-31' AS date), N'[sudz] Q4 2025 / Rslt_26-0212')
...
---------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM sudz.cnInvCmmGr WHERE cnicgKey = 805)
BEGIN
    SET IDENTITY_INSERT sudz.cnInvCmmGr ON;
    INSERT INTO sudz.cnInvCmmGr (cnicgKey, cnicgNmCs, cnicgDate, cnicgName)
    VALUES (805, NULL, '2025-12-31', N'[sudz] Отчёт за IV-й квартал 2025 (Rslt_26-0212), общий');
...
-- yr 900 = 2025
---------------------------------------------------------------------
DELETE FROM sudz.yr_upl_p WHERE yr_upl_p_yr = 900;
IF EXISTS (SELECT 1 FROM sudz.yr WHERE yr_key = 900)
    UPDATE sudz.yr SET yr_CmmGr = NULL WHERE yr_key = 900;
DELETE FROM sudz.yr WHERE yr_key = 900;

SET IDENTITY_INSERT sudz.yr ON;
INSERT INTO sudz.yr (yr_key, yr_variant, cn_inv_dbt_upl, yyyy, yr_CmmGr, yr_Progress)
VALUES (
    900,
    N'[sudz] Основной вариант 2025-го года (S45 / Rslt_26-0212)',
    801,
    27,
    805,
...
---------------------------------------------------------------------
DELETE FROM sudz.DbtValue WHERE dvUpl BETWEEN 801 AND 805;
...
---------------------------------------------------------------------
DELETE FROM sudz.DbtUplCstAg WHERE ducaUpl BETWEEN 801 AND 805;
...
FROM sudz.cn_inv_dbt_upl WHERE upl_key BETWEEN 801 AND 805
UNION ALL
SELECT 'yr', yr_key, CAST(cn_inv_dbt_upl AS varchar(10)), CAST(yr_CmmGr AS varchar(10))
FROM sudz.yr WHERE yr_key = 900;
SELECT dvDbt, dvUpl, LEFT(dvDocBase, 20) AS doc, dvTtl, dvOverd
FROM sudz.DbtValue WHERE dvUpl BETWEEN 801 AND 805 ORDER BY dvDbt, dvUpl;
GO
