...
**Отдельный подтип P1b (7454 / `ГОС.ПОШЛИНА`):** две строки Value с одной суммой на **разных** `iKey` (**91571** / **91572**, слоты **12579** / **12580**).

Корень (подтверждено 2026-09-14): в домене **два** договора `Б/Н` без даты — `cn_key` **2535** и **2536** (оба на БУРГЕОКОМ). `existList`/`#sudzEia` дают **две** строки с одинаковыми excel-ключами; `sqlTblSudzEiaJoin` **без** `cn_key` → одна Tbl-строка матчит оба `iKey`. `A1` (same-iKey sibling) **не** ловит.

Контроль: после `DELETE DbtValue` на 12580 шаг `dbtValueLoad@902` (base=901) **снова** вставил Value@902 на 12580. На **901** один `dbtValueLoad` не пишет tail (`base=curr`).

Data-fix DEV (2026-09-14): снят Value + `invDbtDbtVar` + `invDbtDbt` у **12580**; канон **12579/91571**. Повторный `dbtValueLoad@902` — tail insert=0, 12580 пуст. Полный `invDbtLoad` без код-фикса всё ещё риск (calm/F1 по EIA). Код-фикс backlog: уникальность cn в `existList` и/или `cn_key` в EIA-join / anti-dupe cross-iKey.
