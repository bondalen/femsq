...
-- Только INSERT в test_sudz.*; ссылки на уже существующие ключи ags.*
...
DELETE FROM test_sudz.DbtValue;
DELETE FROM test_sudz.invDbtDbtVar;
DELETE FROM test_sudz.invDbtDbt;
DELETE FROM test_sudz.invDbtVar;
DELETE FROM test_sudz.invDbt;
DELETE FROM test_sudz.Dbt;

---------------------------------------------------------------------
-- Dbt: две канонические задолженности
---------------------------------------------------------------------
SET IDENTITY_INSERT test_sudz.Dbt ON;
INSERT INTO test_sudz.Dbt (dbtKey, dbtNote) VALUES
 (82, N'Rslt стр.82 / ciaKey=11002 / СФ 7947 / 1-306-12 / 70525000.01'),
 (85, N'Rslt стр.85 / P1: А19-16343/2021 ↔ 90 / 32-425/05-18 / 9527.42');
SET IDENTITY_INSERT test_sudz.Dbt OFF;

---------------------------------------------------------------------
-- invDbt: слоты у СФ (суррогат idKey)
-- 82 → inv 11885; 85 → inv 12032 (А19) и inv 20505 (90)
---------------------------------------------------------------------
SET IDENTITY_INSERT test_sudz.invDbt ON;
INSERT INTO test_sudz.invDbt (idKey, idInv, idNum, idNote) VALUES
...
SET IDENTITY_INSERT test_sudz.invDbt OFF;

---------------------------------------------------------------------
-- invDbtDbt: красный путь идентичности
---------------------------------------------------------------------
INSERT INTO test_sudz.invDbtDbt (iddInv, iddDbt, iddInvDbt)
VALUES
 (11885, 82, 8201),
 (12032, 85, 8501),
 (20505, 85, 8502);
...
---------------------------------------------------------------------
SET IDENTITY_INSERT test_sudz.invDbtVar ON;
INSERT INTO test_sudz.invDbtVar (idvvKey, idvvCnNum, idvvInvNum, idvvAccnt, idvvCn_s_org) VALUES
...
SET IDENTITY_INSERT test_sudz.invDbtVar OFF;

---------------------------------------------------------------------
-- invDbtDbtVar: фиолетовый мост слот↔контекст
---------------------------------------------------------------------
INSERT INTO test_sudz.invDbtDbtVar (iddvInvDbt, iddvInvDbtVar) VALUES
 (8201, 8201),
 (8501, 8501),
 (8502, 8502);
...
SELECT
    ...
    dv.dvDateMaturity,
    dv.dvDocBase
FROM test_sudz.DbtValue dv
JOIN test_sudz.cn_inv_dbt_upl u ON u.upl_key = dv.dvUpl
