-- =============================================================================
-- S43: порядок колонок date-major (как Excel Rslt) + обёртка Yr_DbtChanges
-- DEV only. sqlcmd -I
-- =============================================================================
...
CREATE OR ALTER PROCEDURE test_sudz.Yr_DbtChanges_mini
    @yr int
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        RAISERROR(N'test_sudz.Yr_DbtChanges_mini: yr_key=%d не найден', 16, 1, @yr);
        RETURN;
    END;
    ...
    DECLARE @sel_block nvarchar(max);
    DECLARE @sql nvarchar(max);
    DECLARE @yrStr nvarchar(20) = CONVERT(nvarchar(20), @yr);
    DECLARE @baseDate date;
    DECLARE @baseDateStr nvarchar(10);
    ...
    SET @baseDateStr = CONVERT(nvarchar(10), @baseDate, 23);
    ...
    /* SELECT-список: внутри каждой даты — порядок как в Excel-блоке */
    ;WITH dates AS (
        SELECT DISTINCT f.upl_date
        FROM test_sudz.vw_Yr_DbtFact f
        WHERE f.yr_key = @yr
    )
    SELECT @sel_block = STUFF((
        SELECT
            N', c.'  + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_cnNumEnum') +
            N', cs.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_csoCnDate') +
            N', iv.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_invNumEnum') +
            N', idn.'+ QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_idNum') +
            N', og.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_org_id_value_l') +
            N', it.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_ITN') +
            N', ct.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_CtptOrg') +
            N', m.'  + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_Maturity') +
            N', t.'  + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_Ttl') +
            N', o.'  + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_Overd') +
            N', ck.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_CstAgPnKey') +
            N', cc.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_CstAgPnCode') +
            N', cn.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_CstAgPnName') +
            N', ag.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_AgOrg') +
            CASE WHEN upl_date = @baseDate THEN N''
                 ELSE N', NULLIF(CONVERT(money, bo.base_Overd - o.' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_Overd')
                    + N'), 0) AS ' + QUOTENAME(CONVERT(nvarchar(10), upl_date, 23) + N'_погашено')
            END
        FROM dates
        ORDER BY upl_date
        FOR XML PATH(''), TYPE
    ...
    IF @cols_Ttl IS NULL
    BEGIN
        RAISERROR(N'test_sudz.Yr_DbtChanges_mini: нет DbtValue для yr_key=%d', 16, 1, @yr);
        RETURN;
    END;
...
cmm AS (
    ...
    FROM test_sudz.yr y
    JOIN test_sudz.cnInvCmm cm ON cm.cnicGroup = y.yr_CmmGr
...
cst_yr AS (
    ...
    FROM test_sudz.yr y
    JOIN test_sudz.cnInvCmmCst cs ON cs.ciccCmmGr = y.yr_CmmGr AND cs.ciccType = 2
...
dt AS (
    ...
    FROM test_sudz.yr y
    JOIN test_sudz.cnInvCmmDt d ON d.cnicdCmmGr = y.yr_CmmGr
    WHERE y.yr_key = ' + @yrStr + N'
...
fn AS (
    ...
    FROM test_sudz.yr y
    JOIN test_sudz.cnInvCmmFn f ON f.cnicfCmmGr = y.yr_CmmGr
    WHERE y.yr_key = ' + @yrStr + N'
...
gr AS (
    ...
    FROM test_sudz.yr y
    JOIN test_sudz.cnInvGr g ON g.cnigCmmGr = y.yr_CmmGr
    JOIN test_sudz.cnInvGrNm n ON n.cnignKey = g.cnigGrName
    WHERE y.yr_key = ' + @yrStr + N'
...
/* Целевое имя (паритет с ags.Yr_DbtChanges) — тонкая обёртка */
CREATE OR ALTER PROCEDURE test_sudz.Yr_DbtChanges
    @yr int
AS
BEGIN
    SET NOCOUNT ON;
    EXEC test_sudz.Yr_DbtChanges_mini @yr = @yr;
END
GO

EXEC test_sudz.Yr_DbtChanges @yr = 901;
GO
