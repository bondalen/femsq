...
SELECT
    ...
    agog.ogNm AS AgOrg
FROM test_sudz.yr y
JOIN test_sudz.yr_upl_p yp ON yp.yr_upl_p_yr = y.yr_key
JOIN test_sudz.cn_inv_dbt_upl u ON u.upl_key = yp.cn_inv_dbt_upl
LEFT JOIN test_sudz.cn_inv_dbt_upl ub ON ub.upl_key = y.cn_inv_dbt_upl
JOIN test_sudz.DbtValue dv ON dv.dvUpl = u.upl_key AND dv.dvDbt IS NOT NULL
JOIN test_sudz.invDbtVar v ON v.idvvKey = dv.dvInvDbtVar
JOIN test_sudz.invDbtDbtVar dvv ON dvv.iddvInvDbtVar = v.idvvKey
JOIN test_sudz.invDbt slot ON slot.idKey = dvv.iddvInvDbt
...
LEFT JOIN test_sudz.DbtUplCstAg duca
...
---------------------------------------------------------------------
IF OBJECT_ID(N'test_sudz.vw_Yr_DbtChanges_mini_2026', N'V') IS NOT NULL
    DROP VIEW test_sudz.vw_Yr_DbtChanges_mini_2026;
GO

CREATE VIEW test_sudz.vw_Yr_DbtChanges_mini_2026
AS
WITH base AS (
    SELECT *
...
FROM test_sudz.vw_Yr_DbtChanges_mini_2026
ORDER BY dbtKey;
EXEC test_sudz.Yr_DbtChanges_mini @yr = 901;
GO
