-- =============================================================================
-- Пересоздание test_sudz.invDbt / invDbtDbt: суррогатный PK на invDbt
-- DEV only. Таблицы пустые — безопасный DROP.
-- Спецификация: 08-target-schema.md §3.1 / §2.2 (S35)
-- =============================================================================

IF OBJECT_ID(N'test_sudz.invDbtDbt', N'U') IS NOT NULL
    DROP TABLE test_sudz.invDbtDbt;
GO

IF OBJECT_ID(N'test_sudz.invDbt', N'U') IS NOT NULL
    DROP TABLE test_sudz.invDbt;
GO

-- Слот «долг у СФ»: суррогат idKey + бизнес-UNIQUE (idInv, idNum)
CREATE TABLE test_sudz.invDbt
(
...
WHERE c.object_id = OBJECT_ID(N'test_sudz.invDbt')
UNION ALL
SELECT N'invDbtDbt', c.name, ty.name, c.is_identity
FROM sys.columns c
JOIN sys.types ty ON ty.user_type_id = c.user_type_id
WHERE c.object_id = OBJECT_ID(N'test_sudz.invDbtDbt')
ORDER BY tbl, col;
GO
