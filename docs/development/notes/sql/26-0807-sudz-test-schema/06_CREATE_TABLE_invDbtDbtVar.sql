-- =============================================================================
-- test_sudz.invDbtDbtVar — мост invDbt ↔ invDbtVar (песочница СУДЗ)
-- DEV only. Спецификация: 08-target-schema.md §2.4 / §4.2
-- =============================================================================

IF OBJECT_ID(N'test_sudz.invDbtDbtVar', N'U') IS NULL
BEGIN
    CREATE TABLE test_sudz.invDbtDbtVar
    (
...
IF NOT EXISTS (
    ...
    WHERE ep.major_id = OBJECT_ID(N'test_sudz.invDbtDbtVar')
      AND ep.minor_id = 0
...
IF OBJECT_ID(N'test_sudz.trg_invDbtDbtVar_NoForeignContext', N'TR') IS NOT NULL
    DROP TRIGGER test_sudz.trg_invDbtDbtVar_NoForeignContext;
GO

CREATE TRIGGER test_sudz.trg_invDbtDbtVar_NoForeignContext
ON test_sudz.invDbtDbtVar
AFTER INSERT, UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    IF EXISTS (
        SELECT 1
        FROM inserted i
        INNER JOIN test_sudz.invDbt s ON s.idKey = i.iddvInvDbt
        INNER JOIN test_sudz.invDbtVar v ON v.idvvKey = i.iddvInvDbtVar
...
WHERE c.object_id = OBJECT_ID(N'test_sudz.invDbtDbtVar')
ORDER BY c.column_id;

SELECT tr.name AS trigger_name
FROM sys.triggers tr
WHERE tr.parent_id = OBJECT_ID(N'test_sudz.invDbtDbtVar');
GO
