...
CREATE PROCEDURE [ags].[spMstrg_2408] 
	-- Параметры
	@ipgCh int,				-- идентификатор цепи инвестиционных программ
	@MounthEndDate date		-- крайний день месяца для которого строится отчёт
AS
BEGIN
	...
	-- 1.1 таблица для заполнения таблицы Access аналогичной *ipgChRsltPlCstPercent*
	...
	-- заполняем табличку для приёма результата возвращаемого функцией *fnIpgChRsltCstUtlPercentBrn_2408* потом будем к ней обращаться трижды
	...
	from ags.fnIpgChRsltCstUtlPercentBrn_2408(@ipgCh)-- f where f.dateRslt = @MounthEndDate странно, увеличивает время с 53 секунд до 1 минуты 26...

	-- ************************************************************************************************************************
	-- ВОЗВРАЩАЕМ РЕКОРДСЕТЫ, всего 6-ть штук *********************************************************************************

	-- ОЧЕНЬ БОЛЬШАЯ табличка, разделенная на две части, чтобы она влезла в таблички Accdess ==================================

	-- 1-й рекордсет. Возвращаем табличку с номером строки и полями начинающимися с 'ag', 'iv', 'ia'. Это первая часть большой таблицы, будет в "ags_fnIpgChRsltCstUtlPercentBrn_2408_tbl_part_1".
	...
	-- 2-й рекордсет. Возвращаем табличку с номером строки и полями НЕ начинающимися с 'ag', 'iv', 'ia'. Это вторая часть большой таблицы, будет в "ags_fnIpgChRsltCstUtlPercentBrn_2408_tbl_part_2".
	...
	-- ОЧЕНЬ БОЛЬШАЯ табличка, разделенная на две части, чтобы она влезла в таблички Accdess. Окончание =======================

	---- 2...

	-- таблички для отчётов БЕЗ ИСТОЧНИКОВ ОСВОЕНИЯ ===========================================================================
	...
	select 
		...
		, z.iv_lim, z.uk_lim
		, IIf(ag_lim > 0, 'агентская', IIf(iv_lim > 0,'инвест.', 'прочая')) AS ipgSh
		-- текущий месяц ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
		, z.ag_PlAccum, z.ag_PlFulfillment, z.ag_PlPz, z.ag_PlOverFulfillment, z.ag_PlPercent
		, isnull(z.ag_PlFulfillment, 0) + isnull(z.ag_PlOverFulfillment, 0) AS ag_PlFulfillmentAll -- !!! посмотреть, есть что то такое наверное
		...
		, isnull(y.ag_PlFulfillment, 0) + isnull(y.ag_PlOverFulfillment, 0) AS PrM_ag_PlFulfillmentAll -- !!! посмотреть, есть что то такое наверное
		...
		, isnull(x.ag_PlFulfillment, 0) + isnull(x.ag_PlOverFulfillment, 0) AS PrM_ag_PlFulfillment2All -- !!! посмотреть, есть что то такое наверное
	...
	from
		(
			...
			SELECT 
				...
				IIf(za.cstAgPnCode = 'всего' And za.branch = 0, 1, 0) AS wr,
				za.np_acceptedTtlAccum
			FROM 
				(
					SELECT 
						p.rowNum, IsNull(p.ogNm,'итого') AS ogNm, p.branch, p.branchName
						...
						, p.ag_PlPercentMinusOverFulf AS ag_PlPz -- здесь нужно подобрать подходящий процент
						...
						, p.ag_inProcessTtlAccum as ag_acceptedNot -- это по моему *рассматриваемое*, нужно заменить. Да это было "на рассмотрении"
						, IIf(p.ag_Pl is null Or p.ag_Pl = 0, Null, p.ag_acceptedTtl/p.ag_Pl) AS ag_PlPz_M
					...
					WHERE 
						...
						-- 06.02.2025 вот здесь выбираем те, где есть какой-то лимит. Ну и ещё принятые затраты без лимитов...
			...
			WHERE IIf(za.cstAgPnCode = 'всего' And za.branch = 0, 1, 0) = 0
			-- текущий месяц ==================================================================================================================================
		) as z
			LEFT JOIN 
				( 
					...
					From
						(
							SELECT 
								...
								IsNull(p.cstAgPnCode, 'всего') AS cstAgPnCode, IIf(p.ag_PlAccum = 0, Null, p.ag_PlAccum) AS ag_PlAccum, 
								IIf(p.ag_PlFulfillment = 0, Null, p.ag_PlFulfillment) AS ag_PlFulfillment, 
								p.ag_PlPercentMinusOverFulf AS ag_PlPz,  -- здесь нужно подобрать подходящий процент
								IIf(p.ag_PlOverFulfillment = 0, Null, p.ag_PlOverFulfillment) AS ag_PlOverFulfillment, p.ag_PlPercent, 
								p.ag_Pl AS ag_Pl_M, p.ag_acceptedTtl AS ag_acceptedTtl_M, 
							...
							WHERE
								p.dateRslt = IIf
									(
										...
										'0001-01-01', 
										EOMONTH(DATEFROMPARTS(Year(@MounthEndDate), Month(@MounthEndDate)-1, 1)) -- крайний день предшествующего месяца
									) 
									And 
									(IsNull(p.ag_lim, 0) + IsNull(p.iv_lim, 0) + IsNull(p.uk_lim, 0)) > 0
						) as zb
					WHERE IIf(zb.cstAgPnCode = 'всего' And zb.branch = 0, 1, 0) = 0
							...
							WHERE 
								p.dateRslt = IIf
									(
										...
										'0001-01-01', 
										EOMONTH(DATEFROMPARTS(Year(@MounthEndDate), Month(@MounthEndDate) - 2, 1)) -- крайний день месяца, два месяца назад
									)
								And (IsNull(p.ag_lim, 0) + IsNull(p.iv_lim, 0 ) + IsNull(p.uk_lim, 0)) > 0
						) AS zc
					WHERE IIf(zc.cstAgPnCode = 'всего' And zc.branch = 0, 1, 0) = 0
	...
	-- 3-й рекордсет. Это будет в "ags_fnIpgChRsltCstUtlPercentBrnRep01_2408_tbl". Это без источников освоения но со стройками.

	select * from @TableFnIpgChRsltCstUtlPercentBrnRep01_2408

	---- 3...

	-- 4-й рекордсет. Это будет в "ags_fnIpgChRsltCstUtlPercentBrnRep02_2408_tbl". Это без источников освоения и без строек.

	SELECT  
		-- общее ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
		o.rowNum, iif(o.ogNm = 'итого', 'а', o.ipgSh) as ipgSh, lmm.lim as limSort
		, iif(o.branchName is null, o.ogNm, null) as ogNm, iif(o.branchName=o.ogNm, 'Центральный офис',o.branchName ) as branchName,  o.lim, o.ag_lim
	...
	UNION 
	select  
		null, 'агентская_', 0, 'Заказчики', null, null, null, null, null, null, null, null, null, null, null, null, null, null
		, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null
		, null, null, null, null, null, null, null, null, null  
	ORDER BY ipgSh, limSort DESC , lim DESC;

	-- таблички для отчётов БЕЗ ИСТОЧНИКОВ ОСВОЕНИЯ. Окончание ================================================================

	-- таблички для отчётов с ИСТОЧНИКАМИ ОСВОЕНИЯ ============================================================================

	-- 4. табличка для отчёта с источниками освоения, первая ступень со стройками
	...
	SELECT 
		...
		, z.ag_Pl_M, z.ag_acceptedTtl_M, z.ag_acceptedNot, z.ag_PlPz_M, z.mn
		, IIf(z.ag_lim > 0, 'агентская', IIf(z.iv_lim > 0, 'инвест.', 'прочая')) AS ipgSh
			...
			FROM 
				(
					SELECT --*
						...
						, IIf(p.ag_PlFulfillment = 0, Null, p.ag_PlFulfillment) AS ag_PlFulfillment
						, IIf(p.ag_PlAccum is null Or p.ag_PlAccum = 0, Null, p.ag_PlFulfillment/p.ag_PlAccum) AS ag_PlPz
						, IIf(p.ag_PlOverFulfillment = 0, Null, p.ag_PlOverFulfillment) AS ag_PlOverFulfillment
						, p.ag_PlPercent
						, p.ag_Pl AS ag_Pl_M
						, p.ag_acceptedTtl AS ag_acceptedTtl_M
						-- не принятое, видимо, не такое...
					...
					FROM @TableFnIpgChRsltCstUtlPercentBrn_2408 as p --ipgChRsltPlCstPercent AS p @ipgCh @MounthEndDate
						LEFT JOIN ags.cstAgPn AS c ON p.cstAgPnCode = c.cstapIpgPnN
					WHERE 
						-- берем, по состоянию на требуемую дату,
	...
	-- 5-й рекордсет. Это будет в "ags_fnIpgChRsltCstUtlPercentBrnRepSrc01_2408".

	select * from @TableFnIpgChRsltCstUtlPercentBrnRepSrc01_2408

	-- 5. табличка для отчёта с источниками освоения, вторая ступень без строек

	-- 6-й рекордсет. Это будет в "ags_fnIpgChRsltCstUtlPercentBrnRepSrc02_2408".

	SELECT  
		o.rowNum, iif(o.ogNm = 'итого', 'а', o.ipgSh) as ipgSh, lmm.lim as limSort, 
		iif(o.branchName is null, o.ogNm, null) as ogNm,
		iif(o.branchName = o.ogNm, 'Центральный офис', o.branchName ) as branchName,  o.lim, o.ag_lim, 
		o.ag_Ful_OverFul, o.ag_LimPc, o.ag_PlOverLimit_, o.iv_lim, o.uk_lim,  
		o.ag_PlAccum, o.ag_PlFulfillment, o.ag_PlPz, o.ag_PlOverFulfillment, o.ag_PlPercent, 
	...
	WHERE o.cstAgPnCode = 'всего'
	UNION select  
		null, 'агентская_', 0, 'Заказчики', null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, 
		null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null    
	--from (select top 1 ipgKey from ags_ipg)
	ORDER BY ipgSh, limSort DESC , lim DESC;

	-- таблички для отчётов с ИСТОЧНИКАМИ ОСВОЕНИЯ. Окончание =================================================================

	-- ВОЗВРАЩАЕМ РЕКОРДСЕТЫ, всего 6-ть штук. Окончание **********************************************************************
	-- ************************************************************************************************************************
