...
/****** Object:  UserDefinedFunction [ags].[fnIpgChRsltCstUtlPercentBrn_2408_ipgSt]    Script Date: 16.04.2026 12:15:28 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	<Получаем освоение лимитов по каждой *Стройке* для *Цепи инвестпрограмм*> для отдельного пункта структуры инвестпрограммы
-- =============================================
CREATE FUNCTION [ags].[fnIpgChRsltCstUtlPercentBrn_2408_ipgSt] 
(
	-- Параметры
...
BEGIN
	...
	from
		(
			...
			from ags.fnIpgChRsltCstUtl2_2408_ipgSt (@ipgChKey, @ipgSt) t --@fnIpgChRsltCstUtl2_ t
	...
	from
		(
			select
				iif (
						month(datefromparts(s.yyyy, s.mNum, 1)) = month(i.ipgStr), -- дата начала ИПг входит в текущий месяц?
						i.ipgStr, -- да, дата начала ИПг входит в текущий месяц
						iif ( -- нет, дата начала ИПг не входит в текущий месяц
								-- дата окончания ИПг входит в текущий месяц?
								month(datefromparts(s.yyyy, s.mNum, 1)) = month(i.ipgEnd), 
								i.ipgEnd, -- да, дата окончания ИПг входит в текущий месяц
								eomonth(datefromparts(s.yyyy, s.mNum, 1)) -- нет, дата окончания ИПг не входит в текущий месяц
							)
					) dateRslt
				, s.ipgKey, s.ipgCount
			from @spIpgChRsltCstUtl3_oneD s
			...
			select 
				...
				iif ( -- процент освоения -------------------------------------------------------------------------------------------------------------------------------
						...
						iif ( -- нет, лимит имеется
								...
								(isnull(w.ag_PlFulfillment, 0) + isnull(w.ag_PlOverFulfillment, 0))/w.ag_lim 
							)
				...
				, iif ( -- *процент освоения*, включая *рассматриваемое* ------------------------------------------------------------------------------------------------
						...
						iif ( -- нет, лимит имеется
								...
								(isnull(w.ag_PlFulfillment, 0) + isnull(w.ag_PlOverFulfillment, 0) + isnull(w.ag_inProcessTtlAccum, 0))/w.ag_lim 
							)
				...
				, iif ( -- процент освоения, включая освоенное сверх лимита ---------------------------------------------------------------------------------------------
						...
						isnull(w.ag_acceptedTtlAccum, 0)/w.ag_lim 
				...
				, iif ( -- *процент освоения*, включая *освоенное сверх лимита* и *рассматриваемое* ---------------------------------------------------------------------
						...
						null,	-- да, лимит отсутствует
						-- нет, лимит имеется, тогда процент освоения равен частному *принятого общего* и *лимита*
						(isnull(w.ag_acceptedTtlAccum, 0) + isnull(w.ag_inProcessTtlAccum, 0))/w.ag_lim 
				...
				, iif (	-- процент выполнения плана без перевыполнения --------------------------------------------------------------------------------------------------
						...
						isnull(w.ag_PlFulfillment, 0)/w.ag_PlAccum 
				...
				, iif (	-- *процент выполнения плана* без *перевыполнения* включая *рассматриваемое* --------------------------------------------------------------------
						...
						iif (
								...
								(isnull(w.ag_PlFulfillment, 0) + isnull(w.ag_inProcessTtlAccum, 0))/w.ag_PlAccum
							)
				...
				, iif (	-- процент выполнения плана ---------------------------------------------------------------------------------------------------------------------
						...
						(isnull(w.ag_PlFulfillment, 0) + isnull(w.ag_PlOverFulfillment, 0))/w.ag_PlAccum 
				...
				, iif (	-- *процент выполнения плана* включая *рассматриваемое* -----------------------------------------------------------------------------------------
						...
						(isnull(w.ag_PlFulfillment, 0) + isnull(w.ag_PlOverFulfillment, 0) + isnull(w.ag_inProcessTtlAccum, 0))/w.ag_PlAccum 
				...
				, iif (	-- *процент выполнения плана* включая всё, даже и *сверхлимит* ----------------------------------------------------------------------------------
						...
						isnull(w.ag_acceptedTtlAccum, 0)/w.ag_PlAccum 
				...
				, iif (	-- *процент выполнения плана* включая всё, даже и *сверхлимит* и включая *рассматриваемое* ------------------------------------------------------
						...
						(isnull(w.ag_acceptedTtlAccum, 0) + isnull(w.ag_inProcessTtlAccum, 0))/w.ag_PlAccum 
				...
				, iif (	-- процент освоения -----------------------------------------------------------------------------------------------------------------------------
						...
						iif ( -- нет, лимит имеется
								...
								1, -- да, тогда процент освоения равен 100%
								-- нет, тогда процент освоения равен частному суммы выполнения и перевыполнения плана и лимита
								(isnull(w.iv_PlFulfillment, 0) + isnull(w.iv_PlOverFulfillment, 0))/w.iv_lim 
							)
				...
				, iif ( -- *процент освоения*, включая *рассматриваемое* ------------------------------------------------------------------------------------------------
						...
						iif ( -- нет, лимит имеется
								...
								1, -- да, тогда процент освоения равен 100%
								-- нет, тогда процент освоения равен частному суммы *выполнения*, *перевыполнения* и *рассматриваемого* и *лимита*
								(isnull(w.iv_PlFulfillment, 0) + isnull(w.iv_PlOverFulfillment, 0) + isnull(w.ia_inProcessTtlAccum, 0))/w.iv_lim 
							)
				...
				, iif ( -- процент освоения, включая освоенное сверх лимита ---------------------------------------------------------------------------------------------
						...
						null,	-- да, лимит отсутствует
						-- нет, лимит имеется, тогда процент освоения равен частному *принятого общего* и *лимита*
						isnull(w.ia_acceptedTtlAccum, 0)/w.iv_lim 
					) ia_percentDev -- процент освоения, включая освоенное сверх лимита. Окончание ----------------------------------------------------------------------
				, iif ( -- *процент освоения*, включая *освоенное сверх лимита* и *рассматриваемое* ---------------------------------------------------------------------
						w.iv_lim is null or w.iv_lim = 0, -- лимит отсутствует?
						null,	-- да, лимит отсутствует
						-- нет, лимит имеется, тогда процент освоения равен частному *принятого общего* и *лимита*
						(isnull(w.ia_acceptedTtlAccum, 0) + isnull(w.ia_inProcessTtlAccum, 0))/w.iv_lim 
				...
				, iif (	-- процент выполнения плана без перевыполнения --------------------------------------------------------------------------------------------------
						...
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному суммы *выполнения плана* и *плана*
						isnull(w.iv_PlFulfillment, 0)/w.iv_PlAccum 
				...
				, iif (	-- *процент выполнения плана* без *перевыполнения* включая *рассматриваемое* --------------------------------------------------------------------
						...
						iif (
								...
								1, -- да, сумма *выполнения* и *рассматриваемого* больше плана, тогда 100%
								-- нет, тогда *процент выполнения плана* равен частному суммы *выполнения плана* и *рассматриваемого* и *плана*
								(isnull(w.iv_PlFulfillment, 0) + isnull(w.ia_inProcessTtlAccum, 0))/w.iv_PlAccum
							)
				...
				, iif (	-- процент выполнения плана ---------------------------------------------------------------------------------------------------------------------
						...
						null,	-- да, план отсутствует
						-- нет, тогда процент выполнения плана равен частному суммы выполнения и перевыполнения плана и плана
						(isnull(w.iv_PlFulfillment, 0) + isnull(w.iv_PlOverFulfillment, 0))/w.iv_PlAccum 
					) iv_PlPercent -- процент выполнения плана. Окончание -----------------------------------------------------------------------------------------------
				, iif (	-- *процент выполнения плана* включая *рассматриваемое* -----------------------------------------------------------------------------------------
						w.iv_PlAccum is null or w.iv_PlAccum = 0, -- план отсутствует?
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному суммы *выполнения*, *перевыполнения* и *рассматриваемого*  и плана
						(isnull(w.iv_PlFulfillment, 0) + isnull(w.iv_PlOverFulfillment, 0) + isnull(w.ia_inProcessTtlAccum, 0))/w.iv_PlAccum 
					) iv_PlPercentInProcess -- *процент выполнения плана* включая *рассматриваемое*. Окончание ----------------------------------------------------------
				, iif (	-- *процент выполнения плана* включая всё, даже и *сверхлимит* ----------------------------------------------------------------------------------
						w.iv_PlAccum is null or w.iv_PlAccum = 0, -- план отсутствует?
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному *принятого общего* и *плана*
						isnull(w.ia_acceptedTtlAccum, 0)/w.iv_PlAccum 
					) ia_percentPlDev -- *процент выполнения плана* включая всё, даже и *сверхлимит*. Окончание ---------------------------------------------------------
				, iif (	-- *процент выполнения плана* включая всё, даже и *сверхлимит* и включая *рассматриваемое* ------------------------------------------------------
						w.iv_PlAccum is null or w.iv_PlAccum = 0, -- план отсутствует?
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному суммы *принятого общего* и *рассматриваемого*  и плана
						(isnull(w.ia_acceptedTtlAccum, 0) + isnull(w.ia_inProcessTtlAccum, 0))/w.iv_PlAccum 
				...
				, iif (	-- процент освоения -----------------------------------------------------------------------------------------------------------------------------
						...
						iif ( -- нет, лимит имеется
								...
								1, -- да, тогда процент освоения равен 100%
								-- нет, тогда процент освоения равен частному суммы выполнения и перевыполнения плана и лимита
								(isnull(w.uk_PlFulfillment, 0) + isnull(w.uk_PlOverFulfillment, 0))/w.uk_lim 
							)
				...
				, iif ( -- *процент освоения*, включая *рассматриваемое* ------------------------------------------------------------------------------------------------
						...
						iif ( -- нет, лимит имеется
								...
								1, -- да, тогда процент освоения равен 100%
								-- нет, тогда процент освоения равен частному суммы *выполнения*, *перевыполнения* и *рассматриваемого* и *лимита*
								(isnull(w.uk_PlFulfillment, 0) + isnull(w.uk_PlOverFulfillment, 0) + isnull(w.uk_inProcessTtlAccum, 0))/w.uk_lim 
							)
				...
				, iif ( -- процент освоения, включая освоенное сверх лимита ---------------------------------------------------------------------------------------------
						...
						null,	-- да, лимит отсутствует
						-- нет, лимит имеется, тогда процент освоения равен частному *принятого общего* и *лимита*
						isnull(w.uk_acceptedTtlAccum, 0)/w.uk_lim 
					) uk_percentDev -- процент освоения, включая освоенное сверх лимита. Окончание ----------------------------------------------------------------------
				, iif ( -- *процент освоения*, включая *освоенное сверх лимита* и *рассматриваемое* ---------------------------------------------------------------------
						w.uk_lim is null or w.uk_lim = 0, -- лимит отсутствует?
						null,	-- да, лимит отсутствует
						-- нет, лимит имеется, тогда процент освоения равен частному *принятого общего* и *лимита*
						(isnull(w.uk_acceptedTtlAccum, 0) + isnull(w.uk_inProcessTtlAccum, 0))/w.uk_lim 
				...
				, iif (	-- процент выполнения плана без перевыполнения --------------------------------------------------------------------------------------------------
						...
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному суммы *выполнения плана* и *плана*
						isnull(w.uk_PlFulfillment, 0)/w.uk_PlAccum 
				...
				, iif (	-- *процент выполнения плана* без *перевыполнения* включая *рассматриваемое* --------------------------------------------------------------------
						...
						iif (
								...
								1, -- да, сумма *выполнения* и *рассматриваемого* больше плана, тогда 100%
								-- нет, тогда *процент выполнения плана* равен частному суммы *выполнения плана* и *рассматриваемого* и *плана*
								(isnull(w.uk_PlFulfillment, 0) + isnull(w.uk_inProcessTtlAccum, 0))/w.uk_PlAccum
							)
				...
				, iif (	-- процент выполнения плана ---------------------------------------------------------------------------------------------------------------------
						...
						null,	-- да, план отсутствует
						-- нет, тогда процент выполнения плана равен частному суммы выполнения и перевыполнения плана и плана
						(isnull(w.uk_PlFulfillment, 0) + isnull(w.uk_PlOverFulfillment, 0))/w.uk_PlAccum 
					) uk_PlPercent -- процент выполнения плана. Окончание -----------------------------------------------------------------------------------------------
				, iif (	-- *процент выполнения плана* включая *рассматриваемое* -----------------------------------------------------------------------------------------
						w.uk_PlAccum is null or w.uk_PlAccum = 0, -- план отсутствует?
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному суммы *выполнения*, *перевыполнения* и *рассматриваемого*  и плана
						(isnull(w.uk_PlFulfillment, 0) + isnull(w.uk_PlOverFulfillment, 0) + isnull(w.uk_inProcessTtlAccum, 0))/w.uk_PlAccum 
					) uk_PlPercentInProcess -- *процент выполнения плана* включая *рассматриваемое*. Окончание ----------------------------------------------------------
				, iif (	-- *процент выполнения плана* включая всё, даже и *сверхлимит* ----------------------------------------------------------------------------------
						w.uk_PlAccum is null or w.uk_PlAccum = 0, -- план отсутствует?
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному *принятого общего* и *плана*
						isnull(w.uk_acceptedTtlAccum, 0)/w.uk_PlAccum 
					) uk_percentPlDev -- *процент выполнения плана* включая всё, даже и *сверхлимит*. Окончание ---------------------------------------------------------
				, iif (	-- *процент выполнения плана* включая всё, даже и *сверхлимит* и включая *рассматриваемое* ------------------------------------------------------
						w.uk_PlAccum is null or w.uk_PlAccum = 0, -- план отсутствует?
						null,	-- да, план отсутствует
						-- нет, тогда *процент выполнения плана* равен частному суммы *принятого общего* и *рассматриваемого*  и плана
						(isnull(w.uk_acceptedTtlAccum, 0) + isnull(w.uk_inProcessTtlAccum, 0))/w.uk_PlAccum 
								...
								, iif ( -- на рассмотрении из недовыполненного ..........................................................................................
										...
										iif ( -- нет, *общее принятое* не больше *лимита*
												...
												iif ( -- нет, *план* не меньше *общего принятого*, тогда есть *недовыполнение*
														...
														-- нет, *недовыполненое* не меньше *рассматриваемого*. 
														-- Тогда *рассматриваемое из недовыплненного* равно *рассматриваемому*
														isnull(ia_inProcessTtlAccum, 0) --isnull(ia_acceptedTtlAccum, 0) 15.04.2025
													)
											)
								...
								(
									...
									from
										(
											...
											select u.dateRslt, 
												iif ( -- если дата меньше последнего дня января, то считаем номер месяца нулевым
											...
											select ddd,
												iif ( -- если дата меньше последнего дня января, то считаем номер месяца нулевым
														ddd < datefromparts(@lsYy, 1, 31), 0, month(ddd) 
													) mNum, ipgKey
											from
												(
													select eomonth(datefromparts(@lsYy, m.mNum, 1)) ddd -- поменял 22-0204
												...
												left join
													(
														...
														from
															(
																select month(t.dateRslt) m, max(t.dateRslt) d 
																from @dt t where t.ipgKey is not null group by month(t.dateRslt)
															) x
										...
										left join
											@spIpgChRsltCstUtl3_oneD i on x.mNum = i.mNum and (x.ipgKey = i.ipgKey or i.ipgKey is null)
											...
											left join -- для агентской схемы
												...
												left join
													(
														select ipgcrChain, ipgcrIpg, iuplpIpgPn, iuplpSubAg
															, iuplpM01, iuplpM02, iuplpM03
														...
														from ags.ipgChRl r
															join ags.ipgUtPlGr g on r.ipgcrUtPlGr = g.iuplgKey
																join ags.ipgUtPlGrP p on g.iuplgKey = p.iuplgpGr
																	join ags.ipgUtPlP n on p.iuplgpPl = n.iuplpPl
