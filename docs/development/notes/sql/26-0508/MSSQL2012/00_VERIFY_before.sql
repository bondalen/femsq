...
-- -----------------------------------------------------------------------------
-- 5. Версия SQL Server (MSSQL2012: 2012 SP4+, major_version >= 11)
-- -----------------------------------------------------------------------------
PRINT '--- 5. SQL Server version ---';
SELECT @@VERSION AS sql_version, SERVERPROPERTY('ProductMajorVersion') AS major_version,
    CASE WHEN CAST(SERVERPROPERTY('ProductMajorVersion') AS int) >= 11
         THEN 'OK for MSSQL2012 package' ELSE 'UNSUPPORTED' END AS package_compat;
GO
