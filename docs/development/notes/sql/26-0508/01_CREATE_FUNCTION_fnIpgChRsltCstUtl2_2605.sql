...
-- =============================================================================
-- Файл:    01_CREATE_FUNCTION_fnIpgChRsltCstUtl2_2605.sql
-- Пакет:   docs/development/notes/sql/26-0508/
-- Объект:  ags.fnIpgChRsltCstUtl2_2605
-- Назначение: Inline TVF — результаты по стройкам (разные схемы) для цепи
--             инвестпрограмм, с опциональной фильтрацией по группе строек.
--
-- Параметры:
--   @ipgChKey  int             — ключ цепи инвестпрограмм
--   @ipgSt     nvarchar(255)   — группа строек из ags.[importIpgSt_26-0320]
--                                (NULL = все строки, без фильтра)
--
-- Зависимости:
--   - ags.fnIpgChRsltCstUtl_2408  (вызывается внутри)
--   - ags.[importIpgSt_26-0320]   (таблица-справочник групп)
--
-- Применимость к продуктивному серверу:
--   CREATE OR ALTER — безопасен для повторного применения.
--   Откат: DROP FUNCTION [ags].[fnIpgChRsltCstUtl2_2605]
--
...
-- =============================================
CREATE OR ALTER FUNCTION [ags].[fnIpgChRsltCstUtl2_2605] 
(	
	@ipgChKey	int,			-- цепь инвестиционных программ
	@ipgSt		nvarchar(255) = NULL	-- пункт структуры инвестпрограммы (NULL = без фильтра)
)
RETURNS TABLE 
AS
RETURN 
(
	...
	-- земельные участки
	, presentedRalp, presentedRalpAccum, acceptedRalp, acceptedRalpAccum, returnedRalp, returnedRalpAccum, inProcessRalp, inProcessRalpAccum, notArrivedRalp, notArrivedRalpAccum
	-- хранение
...
from
	(
		select
			...
			, sum(presentedRalp) as presentedRalp, sum(presentedRalpAccum) as presentedRalpAccum
			, sum(acceptedRalp) as acceptedRalp, sum(acceptedRalpAccum) as acceptedRalpAccum
				...
				where
					not (
							r.lim is null
								...
								and r.presentedRalp is null and r.presentedRalpAccum is null 
								and r.acceptedRalp is null and r.acceptedRalpAccum is null 
...
PRINT 'OK: ags.fnIpgChRsltCstUtl2_2605 создана/обновлена'
GO
