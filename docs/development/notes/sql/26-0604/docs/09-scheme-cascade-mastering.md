...
**Дата:** 2026-06-11  
**Обновлено:** 2026-06-30 (примеры `np_` / `oh_`; agency-golden; Решение 21)  
...
Объекты: `ags.fnMasteringCstAgPn` (legacy, продуктив), `ags.fnMasteringCstAgPn_2606` (клон с `ipgChRl_2606` вместо `ipgChRl`).
...
| **51** | да | OUT_GROUP: тип `2` разрешён в `stIpgOutLimPn_2606`, стройки вне IN_GROUP узла 51 |
...
### Пример 5 — agency-golden: факт и план по `ag_*` (Решение 21)

**Узел структуры:** `stIpg = 4`. **Стройки:** **849** (`051-2000973`) и **1862** (`051-2000860`) — обе **агентская** схема (`ipgpSh = 2`) на ИП 6/8/11.

| `cstAgPn` | Схема | Факт `fn2` (агент.) | План PercentBrn `ag_Pl` | Примечание |
|-----------|-------|---------------------|-------------------------|------------|
| **849** | агент. | крупный, ИП 6→8→11 | ненулевой на 6/8/11 | нативный UtPl + группы 18–20 |
| **1862** | агент. | меньше 849, те же ИП | ненулевой на 6/8/11 | пара в одном `stIpg` |
| **2102** @ 42 | **инвест.** | ≈ 0 | `iv_*` (не `ag_*`) | invest-golden, не agency |

**Ручной прогон:** `@ipgStKey = 4`, `@MounthEndDate = '2022-12-31'` — см. `13-plan-stcost-monthly-acceptance.md` §17–18.

**Не путать с invest-golden 2102** (`stIpg = 42`): только инвестиционная схема, нет динамики агентского факта для приёмки графиков освоения.
