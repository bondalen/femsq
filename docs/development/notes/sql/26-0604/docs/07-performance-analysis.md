...
## 11. Планы UtPl и полнота замеров perf (2026-06-23)

Замеры **К-7** (§8–10) выполнялись на prod-подобном наборе, где **плановые показатели** UtPl для большинства пунктов ИП на контрольную дату **не вычисляются** (нет UtPl на активной ИП). Контур `fnStCostRsIpgPn_2606` / `smmTtl` при этом почти не нагружен.

**Вывод:** достижение К-7 **< 120 с** на `fn2` stIpg=NULL **не гарантирует** укладывание в тот же бюджет после ввода полного помесячного UtPl по цепи. Единственный достоверный источник — **наиболее полный тест** (этап 18.7.4): цепь 5, три ИП, ~1889 пунктов, 17 дат, скрипты `07o` / `run_plan_full_chain5.sh`.

До его PASS заявления о соответствии **процедуры в целом** требованиям по времени следует считать **предварительными**. Детали: `13-plan-stcost-monthly-acceptance.md` §12, `08-testing-strategy.md` (фаза 3).

---

*Файл создан: 2026-06-11. Разделы 8–10: 2026-06-15. Раздел 11: 2026-06-23. Автор: Александр.*
