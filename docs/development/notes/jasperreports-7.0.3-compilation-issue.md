...
**Дата:** 2025-11-28  
**Версия:** 1.1  
**Статус:** Решено (через предкомпиляцию отчётов)
...
- ✅ Fat JAR успешно собирается (≈49–51 МБ)
- ✅ Thin JAR формируется и используется для обновлений
- ✅ DLL библиотеки включены
- ✅ Предкомпиляция отчётов реализована через отдельный класс и Maven plugin
- ✅ Отчёты JasperReports компилируются на этапе сборки и доступны в fat JAR

## Принятое решение

В качестве основного подхода выбран **Вариант 1 (Предкомпиляция отчётов)** — это наиболее надёжное решение, которое:
- Работает в любой среде (в том числе внутри Spring Boot fat JAR)
- Не требует дополнительных зависимостей (Groovy/JavaScript и т.п.)
- Гарантирует компиляцию на этапе сборки и отсутствие ошибок компиляции на runtime

Реализация:

- Создан класс `com.femsq.reports.build.PrecompileReports`, который:
  - Находит JRXML файлы отчётов в каталоге встроенных отчётов
  - Компилирует их в `.jasper` файлы на этапе сборки с полным classpath
  - Сохраняет скомпилированные `.jasper` в целевую директорию ресурсов (например `target/classes/reports/embedded/`)
- В `femsq-reports/pom.xml` настроен `exec-maven-plugin`, который запускает `PrecompileReports` на одной из фаз сборки (process-classes/process-resources).
- В fat JAR включаются как исходные `.jrxml`, так и предкомпилированные `.jasper` файлы, что позволяет:
  - Использовать `.jasper` на runtime
  - Сохранять `.jrxml` для отладки и доработки.

## Следующие шаги (исторические, выполнены)

1. Создать класс `PrecompileReports` для компиляции отчётов на этапе сборки — ✅ выполнено.
2. Настроить Maven plugin для запуска предкомпиляции — ✅ выполнено.
3. Включить `.jasper` файлы в JAR вместе с `.jrxml` — ✅ выполнено.
4. Обновить `ReportGenerationService` для использования предкомпилированных отчётов с fallback на компиляцию `.jrxml` при необходимости — ✅ выполнено.
