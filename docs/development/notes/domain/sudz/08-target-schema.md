...
**S66c (воронка 0069):** последний Access-шаг с прямым аналогом в цели — INSERT `cnInvAccntSmpl`. Дальше Access пишет `cnInvAccnt`/`cn_inv_dbt`; FEMSQ по критерию 0069 должен писать `Dbt`/`invDbt*`/`DbtValue`. Новые stepId **не** выводятся из `cidu*Acc*` без решения владельца. Имя VBA `invDbtDouble` ≠ таблица `invDbt` (QueryDef `invDoubleCia` на `cnInvAccnt`).
...
- Процесс: внесение/контроль в **1.1.2.2**; **1.1.3** — веха готовности к исходящим (уточнено **S60**: прекращение правок, без операции New→`yr_CmmGr`); длинный S25 убран из 03 → [04 §2.6](./04-data-model.md).
- DDL колонки — при реализации Rslt повтор / CRUD yr (ещё не в БД).

### S60 — 2026-08-09 (закрытие 0070)

- Веха **1.1.3** = решение оператора прекратить правки комментариев; операции New→`yr_CmmGr` **нет**.
- Полный путь папки выгрузки — **отложен** до отдельного решения.
- Задача **0070** → completed (Progress MVP).
...
- **S59b (приёмка владельцем):** срез **D644 / Свод · Выгрузить** принят («пока приемлемо»); адаптивная высота строк по длине «Комментарий/меры»; оговорки — полные суммы свода после seed портфеля.
- **S60:** 0070 закрыт; веха 1.1.3 без New→CmmGr; полный путь папки — отдельно.
- Progress: операции **D644 · Выгрузить** / **Свод · Выгрузить** (срез = `currUpl`; `yr_CmmGr`).
