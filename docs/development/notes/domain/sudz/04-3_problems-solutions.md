...
**Последнее обновление:** 2026-08-26 (S73: зерно `(iKey,ciaName)` / счёт долгов)  
**Статус:** рабочий реестр (нарастающий)  
**План чата:** [chat-plan-26-0802-sudz.md](../../chats/chat-plan/chat-plan-26-0802-sudz.md)  
**Контекст модели:** [04-data-model.md](./04-data-model.md) · пример [04-2](./04-2_example-rslt-82-85.md) · зерно/счёт [04-4](./04-4_legacy-debt-grain.md) · **физ. схема + ER:** [08-target-schema.md](./08-target-schema.md)
...
**Статус:** **закрыта целевой моделью** (`Dbt` / `invDbt` / `invDbtDbt`, S14–S32). Исторический костыль **`ciaName` как колонка** в целевую схему **не** переносится (S33); роль дискриминатора на СФ — **`invDbt.idNum`** (seed: `idNum ← ciaName` для named, S73).

**S73 (счёт / зерно старой структуры):** операционное зерно долга — **`(iKey, ciaNameNull)`**, не `ciaKey` и не тройка Rslt с СГК. Оценка числа долгов на dev-срезе: **11 906–11 907** (база 11 897 + сплит 9 «скачок порядка» ± 1 спорный `4480`); **10** кейсов на ручное подтверждение. СГК — атрибут снимка (брожение доказано: 0 concurrent multi-acc в одной upl). Подробно: [04-4](./04-4_legacy-debt-grain.md).
...
| `invDbt.idNum` | 2022-12-06 | Несколько долгов на один `idInv`; в `invDbtTmpCiaRel` для named **`idNum = ciaName`** | **структурно да**; живой квартальный контур после 2022 шёл через `ciaName`, не через наполнение `invDbtValue` |
| Ручное «уточнение основания» в Rslt | практика | Различение глазами | **нет** |
| Целевая модель `Dbt`+`invDbt`+`invDbtDbt` | S14–S32 | Каждый долг — отдельная сущность `Dbt`; на одном `Inv` допустимо N слотов `invDbt` без текстового дискриминатора | **да** — колонка `ciaName` больше не нужна; смысл — в `idNum` / `Dbt` |
...
## 10. `DbtValue` без прямого FK на `Dbt` (M2 / S71)

**Дата:** 2026-08-25  
**Спека:** [08-target-schema §2.5](./08-target-schema.md) · эскиз [26-0824-…dbtvar.png](./assets/26-0824-sudz-target-sketch-dbtvar.png)

### 10.1. Разночтение

| Источник | Связь `Dbt`↔`DbtValue` |
|----------|------------------------|
| Эскиз S32 / `26-0824` | **Нет** прямой стрелки; `DbtValue` → `invDbtVar` (+ upl) |
| Текст S32 (§7.9) | Величина + FK на `invDbtVar` + `upl`; про `dvDbt` не сказано |
| DDL S38 / витрины | Есть **`dvDbt`**, `UNIQUE(dvDbt, dvUpl)`, триггер требует `invDbtDbt` |

Прямой FK — отклонение реализации от эскиза (удобство отчётов + наследие S14).

### 10.2. Почему нельзя «только var + upl»

`invDbtVar` M:N к слотам. При **P2** несколько долгов в одной выгрузке могут делить один вариант контекста → `UNIQUE(var, upl)` ломает P2. Нужен якорь **слота** или **канона**.

### 10.3. Решение владельца — **M2**

| Правило | Содержание |
|---------|------------|
| FK на `Dbt` | **Нет** (`dvDbt` снимается из целевой модели) |
| Якорь величины | **`dvInvDbt`** → `invDbt` (+ `dvInvDbtVar`, `dvUpl`) |
| UNIQUE | `(dvInvDbt, dvUpl)` |
| `Dbt` | **Обязателен** для каждой задолженности; к величине — через `invDbt` → `invDbtDbt` → `Dbt` |
| Триггер Value | Согласовать слот↔var (+ membership cnInv); **не** требовать `invDbtDbt` при INSERT Value |
| Отчёты | `dbtKey` из join через мост; строки без моста — ещё не в зерне Rslt |

Отклонены: **M1** (оставить `dvDbt`), **M3** (якорь `invDbtDbt` — слишком рано жёстко к канону).

### 10.4. Воронка

- Чекбокс **`invDbtLoad` (7):** по-прежнему слой слота/`invDbtVar`/мост var + очередь; **не** реализация `Dbt`/`invDbtDbt`/`DbtValue` в этом шаге (сегм. 15).
- Модель M2 **позволяет** писать `DbtValue` до `Dbt` на **позднейших** шагах; порядок появления Value vs Dbt в воронке — отдельное решение при проектировании тех чекбоксов.
- Миграция физического DDL/`vw_Yr_DbtFact`/seed — **backlog**, не блокер UAT ensure/load 910.

### S73 — 2026-08-26

- Зерно legacy и счёт долгов: [04-4_legacy-debt-grain.md](./04-4_legacy-debt-grain.md). `(iKey, ciaName) ≈ (idInv, idNum)`; СГК вне ключа; **11 906–11 907** долгов; **10** ручных кейсов. Уточнён статус P2 / `idNum` в §1.
