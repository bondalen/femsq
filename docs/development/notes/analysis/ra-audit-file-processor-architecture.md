...
### 10.5. Ошибки формата ячеек (Stage 1, задача 0045)

Ошибка типа (текст в integer-колонке и т.п.) **не должна** прерывать весь прогон. Правила:

- **`rcm_required = 0`:** warning в `adt_results`, поле → `NULL`, **строка принимается** в staging.
- **`rcm_required = 1`:** warning, **строка пропускается**.

Данные Excel не исправляются программой — пользователь правит файл по сообщению в логе. Подробности: `ra-execution-operations.md` → «Ошибки формата ячеек Excel», chat-plan фаза **7.3**.
