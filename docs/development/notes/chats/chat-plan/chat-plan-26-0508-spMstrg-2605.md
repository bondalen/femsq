# План работы чата: Рефакторинг отчётной ветки spMstrg → версия _2605

**Дата:** 2026-05-08  
**Обновлён:** 2026-06-04 (финал — добавлен раздел «Вскрытые архитектурные проблемы»)  
**Автор:** Александр  
**Связанные задачи:** (будут созданы в ходе работы)  
**Связанное резюме:** `docs/development/notes/chats/chat-resume/chat-resume-26-0508-spMstrg-2605.md`  
**Чеклист дня деплоя:** `docs/deployment/db-upgrade-spMstrg-2605-deploy-day-checklist.md`

---

## Контекст

В результате параллельного развития двух проектов — **MS Access + MS SQL Server** (действующая система) и **FEMSQ** (новый проект) — сложились дублирующиеся группы артефактов SQL Server, которые выполняют аналогичные задачи формирования наборов данных для отчётов.

### Цель чата

Разработать и верифицировать SQL-объекты серии `_2605` на тестовой БД **FishEye**, и **сформировать пакет для продуктивного сервера** — набор нумерованных `.sql`-скриптов и документ `docs/deployment/db-upgrade-spMstrg-2605.md` с пошаговым порядком проведения работ, пригодный для самостоятельного применения на продуктиве.

### Существующие артефакты (папка `docs/development/notes/sql/26-0416`)

**Функции:**
- `ags.fnIpgChRsltCstUtl2_2408` — базовая функция: результаты по стройкам (разные схемы) для цепи инвестпрограмм
- `ags.fnIpgChRsltCstUtl2_2408_ipgSt` — тонкая обёртка над предыдущей с JOIN к `importIpgSt_26-0320` и фильтром по `@ipgSt`
- `ags.fnIpgChRsltCstUtlPercentBrn_2408` — большая аналитическая функция (полный набор данных для отчётов); источник для процедур
- `ags.fnIpgChRsltCstUtlPercentBrn_2408_ipgSt` — ~261 КБ, почти полная копия предыдущей; единственное отличие — в строке FROM вызывает `fnIpgChRsltCstUtl2_2408_ipgSt` вместо `fnIpgChRsltCstUtl2_2408`

**Процедуры:**
- `ags.spMstrg_2408` — возвращает 7 рекордсетов клиенту MS Access
- `ags.spMstrg_2408_ipgSt` — то же, но с фильтром по группе строек (`@ipgSt` захардкожен внутри как `'12ОПР'`)
- `ags.spMstrg_2408_SaveToTables` — заполняет 7 таблиц SQL Server (для FEMSQ/JasperReports, обходит ограничение таймаута DBHub)
- `ags.spMstrg_2408_SaveToTables_ipgSt` — то же, но с фильтром по группе строек

**Справочник группировки строек:**
- `ags.[importIpgSt_26-0320]` — таблица (`cst`, `cst_type`); 780 строек, 6 групп (`99ОСТ`, `23КПР`, `20ПКП`, `12ОПР`, `11ПРЗ`, `30ППР`)

**Клиенты процедур:**
- **MS Access** (VBA, `Form_ipgChMin.cls`, `btnMasteringPercent_2408_Click`) — вызывает `spMstrg_2408` или `spMstrg_2408_ipgSt` (выбор — копипаст из комментария; `@ipgSt` группа захардкожена внутри SP); получает рекордсеты и пишет в локальные таблицы Access
- **FEMSQ / JasperReports** (Java) — вызывает `spMstrg_2408_SaveToTables`; отчёт читает из таблиц SQL Server напрямую

### Проблемы

1. **Критическое дублирование:** `fnIpgChRsltCstUtlPercentBrn_2408_ipgSt` (~261 КБ) — почти полная копия `fnIpgChRsltCstUtlPercentBrn_2408`. Изменение логики требует синхронного правки двух объектов.
2. **Дублирование процедур:** логика 7 рекордсетов существует в четырёх экземплярах (`spMstrg_2408`, `_ipgSt`, `_SaveToTables`, `_SaveToTables_ipgSt`).
3. **Захардкоженные параметры в VBA:** имя процедуры и группа строек не параметризованы — выбираются вручную через копипаст.
4. **Отсутствие параметра группы в UI:** нет элемента управления для выбора `@ipgSt` на форме Access.

---

## Принятые решения

- **Именование новых объектов:** суффикс `_2605` (май 2026 — дата архитектурной ревизии), аналогично сложившейся конвенции `_2408`
- **Стратегия перехода:** параллельная разработка; старые `_2408`-объекты сохраняются до финального тестирования новых `_2605`-объектов, после чего удаляются
- **Объекты `_ipgSt`** не получают аналогов в `_2605`-серии — параметр `@ipgSt` встраивается в основные объекты
- **Справочник `importIpgSt_26-0320`** уже присутствует на продуктивном сервере — скрипт его создания/заполнения в пакет для продуктива не включается
- **Удаление устаревших `_2408`/`_ipgSt` объектов** выполняется отдельно, после подтверждения работоспособности `_2605` — оформляется отдельным документом, не входит в основной пакет
- **Изменения в MS Access** (форма, VBA) выполняются вручную; инструкция включается в документ порядка работ

### Целевая архитектура

```
importIpgSt_26-0320  (справочник групп, уже существует на продуктиве)
         ↓ JOIN (внутри функции, условно при @ipgSt IS NOT NULL)
fnIpgChRsltCstUtl2_2605 (@ipgChKey, @ipgSt nvarchar(255) = NULL)
         ↓
fnIpgChRsltCstUtlPercentBrn_2605 (@ipgChKey, @ipgSt nvarchar(255) = NULL)
         ↓
spMstrg_2605 (@ipgCh, @MounthEndDate, @ipgSt = NULL, @saveToTables bit = 0)
    ├── @saveToTables = 0  →  SELECT ×7  →  MS Access (рекордсеты → локальные таблицы)
    └── @saveToTables = 1  →  INSERT ×7  →  FEMSQ (JasperReports читает таблицы SQL Server)

Форма Access:
    cbxIpgSt [(все стройки) / 99ОСТ / 23КПР / 20ПКП / 12ОПР / 11ПРЗ / 30ППР]
         ↓
    btnMasteringPercent_2408_Click → spMstrg_2605 с динамическим @ipgSt
```

### Объекты к удалению (отдельным этапом, после приёмки _2605)

- `fnIpgChRsltCstUtl2_2408_ipgSt`
- `fnIpgChRsltCstUtlPercentBrn_2408_ipgSt`
- `spMstrg_2408_ipgSt`
- `spMstrg_2408_SaveToTables_ipgSt`
- `spMstrg_2408` (заменяется `spMstrg_2605` с `@saveToTables = 0`)
- `spMstrg_2408_SaveToTables` (заменяется `spMstrg_2605` с `@saveToTables = 1`)

---

## Состав пакета для продуктивного сервера

Файлы располагаются в `docs/development/notes/sql/26-0508/`.  
Нумерация префиксов отражает порядок применения.

| Файл | Содержимое | Создаётся на этапе |
|------|-----------|-------------------|
| `00_VERIFY_before.sql` | SELECT-проверки до применения: объекты, строки ResultSet-таблиц, справочник | Этап 0 |
| `01_CREATE_FUNCTION_fnIpgChRsltCstUtl2_2605.sql` | `CREATE OR ALTER FUNCTION` (идемпотентный) | Этап 1 |
| `02_CREATE_FUNCTION_fnIpgChRsltCstUtlPercentBrn_2605.sql` | `CREATE OR ALTER FUNCTION` (идемпотентный) | Этап 2 |
| `03_CREATE_PROCEDURE_spMstrg_2605.sql` | `CREATE OR ALTER PROCEDURE` (идемпотентный) | Этап 3 |
| `04_VERIFY_after.sql` | SELECT-проверки после применения: тест COUNT по режимам `@ipgSt`; ожидаемые значения в комментариях | Этапы 1–3 (наращивается) |
| `05_ROLLBACK.sql` | DROP _2605 объектов — возврат к состоянию до применения | Параллельно этапам 1–3 |
| `06_DROP_obsolete_2408.sql` | DROP устаревших _2408/_ipgSt объектов — **отдельный документ, применяется позже** | Этап 6 |

**Требования к оформлению каждого скрипта:**
- Заголовок-комментарий: версия, дата, автор, назначение
- Идемпотентность: `CREATE OR ALTER` для функций/процедур
- PRINT-маркеры этапов для вывода в sqlcmd
- Транзакционная обёртка там, где возможно и целесообразно

**Итоговый документ порядка работ:** `docs/deployment/db-upgrade-spMstrg-2605.md`

---

## Структурный план

### Этап 0 — Подготовка и верификация ✅

- [x] **0.1 Проверка текущего состояния БД** ✅
  - [x] 0.1.1 Убедиться, что базовые `_2408`-объекты существуют в БД (`FishEye`, схема `ags`):  
    `fnIpgChRsltCstUtl2_2408`, `fnIpgChRsltCstUtlPercentBrn_2408`, `spMstrg_2408`, `spMstrg_2408_SaveToTables`  
    **Исполнено:** Все 4 объекта подтверждены через DBHub (`sys.objects`). `_ipgSt`-варианты на FishEye не применялись — на продуктиве присутствуют; не требуются для разработки `_2605`.
  - [x] 0.1.2 Убедиться, что таблица `importIpgSt_26-0320` заполнена (780 строк, 6 групп)  
    **Исполнено:** Таблица на FishEye отсутствовала. Применён скрипт `26-0416/CREATE_TABLE_ags_importIpgSt_26-0320.sql` через DBHub (CREATE TABLE) + sqlcmd (INSERT, 3 батча по ~400 строк). Результат: **780 строк, 6 групп** (`11ПРЗ`, `12ОПР`, `20ПКП`, `23КПР`, `30ППР`, `99ОСТ`).
  - [x] 0.1.3 Убедиться, что таблицы `spMstrg_2408_ResultSet1..7` существуют и доступны  
    **Исполнено:** Все 7 таблиц подтверждены через `sys.tables`. Количество столбцов: RS1=398, RS2=179, RS3=220, RS4=44, RS5=44, RS6=51, RS7=51.
  - [x] 0.1.4 Зафиксировать эталонные результаты  
    **Исполнено:** Эталонные строки зафиксированы: RS1–3=12693, RS4=744, RS5=32, RS6=**721**, RS7=32. Значение RS6=721 (не 32, как ожидалось) — фактическое состояние таблицы на момент проверки. Будет использовано как эталон при сравнении `_2605`.

- [x] **0.2 Изучение исходного кода `fnIpgChRsltCstUtl2_2408`** ✅
  - [x] 0.2.1 Получить `OBJECT_DEFINITION` функции из БД  
    **Исполнено:** Определение получено через `OBJECT_DEFINITION(OBJECT_ID(...))` via DBHub.
  - [x] 0.2.2 Подтвердить тип функции — **inline TVF**  
    **Исполнено:** `sys.objects.type_desc = 'SQL_INLINE_TABLE_VALUED_FUNCTION'`. Подтверждено.
  - [x] 0.2.3 Выявить точку добавления фильтра по `cstAgPnCode`  
    **Исполнено:** Поле `cstAgPnCode` присутствует в SELECT и GROUP BY подзапроса `asd`. Также изучен `fnIpgChRsltCstUtl2_2408_ipgSt` — он делает `JOIN ags.[importIpgSt_26-0320] s ON f.cstAgPnCode = s.cst WHERE s.cst_type = @ipgSt`. В `_2605` фильтр встраивается в тело основной функции через `WHERE (@ipgSt IS NULL OR EXISTS(...))` в конце внешнего SELECT блока RETURN.

---

### Этап 1 — Создание `fnIpgChRsltCstUtl2_2605` ✅

- [x] **1.1 Проектирование** ✅
  - [x] 1.1.1 Определить способ условной фильтрации по `@ipgSt`:
    - При `@ipgSt IS NULL` — возвращать все стройки (поведение как у `_2408`)
    - При `@ipgSt IS NOT NULL` — фильтр через `WHERE (@ipgSt IS NULL OR EXISTS(SELECT 1 FROM ags.[importIpgSt_26-0320] s WHERE s.cst = asd.cstAgPnCode AND s.cst_type = @ipgSt))`  
    **Исполнено:** выбран `WHERE EXISTS` в конце RETURN-блока — после подзапроса `asd`. Изучен `_ipgSt`-вариант (join-обёртка); в `_2605` фильтр встроен в тело функции.
  - [x] 1.1.2 Оценить влияние условного предиката на план выполнения (inline TVF — оптимизатор справляется)  
    **Исполнено:** для inline TVF оптимизатор раскрывает `EXISTS` в предикат; при `NULL` condition eliminates субзапрос. Признано допустимым.

- [x] **1.2 Создание функции в БД и оформление артефакта** ✅
  - [x] 1.2.1 Написать скрипт `CREATE OR ALTER FUNCTION ags.fnIpgChRsltCstUtl2_2605`  
    **Исполнено:** скрипт построен на основе `OBJECT_DEFINITION(fnIpgChRsltCstUtl2_2408)` с добавлением параметра `@ipgSt nvarchar(255) = NULL` и WHERE-блока перед закрывающей скобкой RETURN. В ходе работы исправлено 5 артефактов извлечения текста (стык 2000-символьных чанков).
  - [x] 1.2.2 Сохранить в `docs/development/notes/sql/26-0508/01_CREATE_FUNCTION_fnIpgChRsltCstUtl2_2605.sql`  
    **Исполнено:** файл создан, 18 345 символов. Заголовок с описанием, SET-операторы, PRINT-маркер.
  - [x] 1.2.3 Выполнить скрипт в FishEye  
    **Исполнено:** `OK: ags.fnIpgChRsltCstUtl2_2605 создана/обновлена` — без ошибок.
  - [x] 1.2.4 Добавить DROP в `05_ROLLBACK.sql`: `DROP FUNCTION IF EXISTS ags.fnIpgChRsltCstUtl2_2605`  
    **Исполнено:** строка присутствует в `05_ROLLBACK.sql` (DROP всех трёх объектов `_2605`) ✅

- [x] **1.3 Тестирование + наполнение `04_VERIFY_after.sql`** ✅
  - [x] 1.3.1 Режим «все стройки» (`@ipgSt = NULL`): COUNT строк = COUNT из `fnIpgChRsltCstUtl2_2408`  
    **Исполнено:** `fn2408(15)` = 14210, `fn2605(15, NULL)` = 14210 — **совпадение точное** ✅
  - [x] 1.3.2 Режим фильтрации (`@ipgSt = '12ОПР'`): сравнить с `fnIpgChRsltCstUtl2_2408_ipgSt`  
    **Исполнено:** `fn2605(15, '12ОПР')` = 661 строк. `_ipgSt`-вариант на FishEye не установлен (только на продуктиве — по плану). Результат 661 является подмножеством 14210; в `importIpgSt_26-0320` группа `12ОПР` = 61 строек. ✅
  - [x] 1.3.3 Проверить количество столбцов  
    **Исполнено:** `fn2408` = 90 столбцов, `fn2605` = 90 столбцов, разница = 0 ✅
  - [x] 1.3.4 Успешные проверочные запросы перенести в `04_VERIFY_after.sql` (секция «fn_2605»)  
    **Исполнено:** секции 2a (столбцы), 2c (COUNT fn2: 14210/14210/661) в `04_VERIFY_after.sql` ✅

---

### Этап 2 — Создание `fnIpgChRsltCstUtlPercentBrn_2605` ✅

- [x] **2.1 Проектирование** ✅
  - [x] 2.1.1 Взять за основу `fnIpgChRsltCstUtlPercentBrn_2408` из `26-0416/`  
    **Исполнено:** функция — multi-statement TVF (260 851 символ), источник данных получается из OBJECT_DEFINITION в момент выполнения скрипта.
  - [x] 2.1.2 Добавить параметр `@ipgSt nvarchar(255) = NULL`  
    **Исполнено:** добавлен через REPLACE прямо внутри SQL-скрипта.
  - [x] 2.1.3 Изменить строку источника данных (единственное содержательное изменение):
    - Было: `FROM ags.fnIpgChRsltCstUtl2_2408(@ipgChKey) t`
    - Стало: `FROM ags.fnIpgChRsltCstUtl2_2605(@ipgChKey, @ipgSt) t`  
    **Исполнено:** одна замена через REPLACE.
  - [x] 2.1.4 Убедиться, что `cst_type` из `_2605`-функции не конфликтует с именами столбцов результата  
    **Исполнено:** `_2605` не добавляет `cst_type` в результирующий набор (столбцов 90, использует WHERE EXISTS). Конфликт отсутствует.

- [x] **2.2 Создание функции в БД и оформление артефакта** ✅
  - [x] 2.2.1 Написать скрипт `CREATE OR ALTER FUNCTION ags.fnIpgChRsltCstUtlPercentBrn_2605`  
    **Исполнено:** скрипт применяет 3 замены к `OBJECT_DEFINITION(_2408)` через `sp_executesql`. Содержит проверку IS NULL и контроль замен.
  - [x] 2.2.2 Сохранить в `docs/development/notes/sql/26-0508/02_CREATE_FUNCTION_fnIpgChRsltCstUtlPercentBrn_2605.sql`  
    **Исполнено:** файл создан.
  - [x] 2.2.3 Выполнить скрипт в FishEye  
    **Исполнено:** `OK: ags.fnIpgChRsltCstUtlPercentBrn_2605 создана/обновлена` — без ошибок. Повторное применение проверено (идемпотентность ✅).
  - [x] 2.2.4 Добавить DROP в `05_ROLLBACK.sql`  
    **Исполнено:** запись добавлена в `05_ROLLBACK.sql`.

- [x] **2.3 Тестирование** ✅
  - [x] 2.3.1 Режим «все стройки» (`@ipgSt = NULL`): сравнить COUNT с `fnIpgChRsltCstUtlPercentBrn_2408`  
    **Исполнено:** `fnPBrn_2408(15)` = 12693, `fnPBrn_2605(15, NULL)` = 12693 — **совпадение точное** ✅
  - [x] 2.3.2 Режим фильтрации (`@ipgSt = '12ОПР'`)  
    **Исполнено:** `fnPBrn_2605(15, '12ОПР')` = 604 строки. Является подмножеством 12693. ✅
  - [x] 2.3.3 Проверка состава столбцов  
    **Исполнено:** `fnPBrn_2408` = 398 столбцов, `fnPBrn_2605` = 398 столбцов, разница = 0 ✅
  - [x] 2.3.4 Успешные проверочные запросы перенести в `04_VERIFY_after.sql` (секция «fnPBrn_2605»)  
    **Исполнено:** секции 2b (столбцы), 2d (COUNT fnPBrn: 12693/12693/604) в `04_VERIFY_after.sql` ✅

---

### Этап 3 — Создание `spMstrg_2605` ✅

- [x] **3.1 Проектирование** ✅
  - [x] 3.1.1 Определить сигнатуру:
    ```sql
    CREATE OR ALTER PROCEDURE ags.spMstrg_2605
        @ipgCh         int,
        @MounthEndDate date,
        @ipgSt         nvarchar(255) = NULL,  -- NULL = все стройки
        @saveToTables  bit           = 0      -- 0 = SELECT (Access), 1 = INSERT (FEMSQ)
    ```
    **Исполнено:** сигнатура реализована именно так.
  - [x] 3.1.2 Определить структуру тела:
    - Заполнить табличную переменную из `fnIpgChRsltCstUtlPercentBrn_2605(@ipgCh, @ipgSt)`
    - Для каждого из 7 рекордсетов: `IF @saveToTables = 1 → TRUNCATE + INSERT INTO ResultSet_N; ELSE → SELECT`
    **Исполнено:** реализовано через T-SQL трансформацию `OBJECT_DEFINITION(spMstrg_2408_SaveToTables)` + CHARINDEX/STUFF для 7 IF/ELSE блоков и TRUNCATE секции.
  - [x] 3.1.3 Сохранить PRINT-операторы логирования времени шагов  
    **Исполнено:** все PRINT-операторы из `spMstrg_2408_SaveToTables` сохранены (логирование тайминга по каждому из 7 шагов).

- [x] **3.2 Создание процедуры в БД и оформление артефакта** ✅
  - [x] 3.2.1 Написать скрипт `CREATE OR ALTER PROCEDURE ags.spMstrg_2605`  
    **Исполнено:** скрипт применяет трансформации к `OBJECT_DEFINITION(_SaveToTables)` через CHARINDEX+STUFF в SQL Server, без извлечения в Python (избегаем артефактов чанкования). Длина результирующего SQL = 82439 символов.
  - [x] 3.2.2 Сохранить в `docs/development/notes/sql/26-0508/03_CREATE_PROCEDURE_spMstrg_2605.sql`  
    **Исполнено:** файл создан.
  - [x] 3.2.3 Выполнить скрипт в FishEye  
    **Исполнено:** `ags.spMstrg_2605 создана успешно!` — без ошибок.
  - [x] 3.2.4 Добавить DROP в `05_ROLLBACK.sql`  
    **Исполнено:** `05_ROLLBACK.sql` уже содержал эту строку (был подготовлен заранее).

- [x] **3.3 Тестирование режима Access (`@saveToTables = 0`)** ✅
  - [x] 3.3.1 Вызвать с `@ipgSt = NULL`: убедиться в 7 рекордсетах  
    **Исполнено:** процедура вернула RS1 (12693 строк, все месяцы), RS2-RS7 — данные есть, 7 рекордсетов ✅
  - [x] 3.3.2 Вызвать с `@ipgSt = '12ОПР'`: проверить фильтрацию  
    **Исполнено:** 1812 строк в суммарном выводе (vs ~13000 без фильтра), фильтрация подтверждена ✅
  - [x] 3.3.3 Сравнить COUNT с `spMstrg_2408_SaveToTables`  
    **Исполнено:** RS1=RS2=RS3=12693, RS4=RS6=0, RS5=RS7=1 — **совпадает точно** с эталоном `spMstrg_2408_SaveToTables` ✅

- [x] **3.4 Тестирование режима FEMSQ (`@saveToTables = 1`)** ✅
  - [x] 3.4.1 Вызвать с `@ipgSt = NULL`, зафиксировать время выполнения  
    **Исполнено:** общее время ~16 664 мс. TRUNCATE выполнен (43 мс), 12693 записей в таблице функции ✅
  - [x] 3.4.2 Проверить таблицы `spMstrg_2408_ResultSet1..7`: COUNT  
    **Исполнено:** RS1=12693, RS2=12693, RS3=12693, RS4=0, RS5=1, RS6=0, RS7=1 ✅
  - [x] 3.4.3 Сравнить с эталоном из п. 0.1.4  
    **Исполнено:** полное совпадение с `spMstrg_2408_SaveToTables` ✅
  - [x] 3.4.4 Вызвать с `@ipgSt = '12ОПР'`: убедиться в фильтрации в таблицах  
    **Исполнено:** RS1=RS2=RS3=604 (из 12693), фильтрация работает ✅

- [x] **3.5 Наполнение `04_VERIFY_after.sql`** ✅
  - [x] 3.5.1 Добавить все тестовые запросы  
    **Исполнено:** файл `04_VERIFY_after.sql` создан и содержит все 6 тестовых секций.

- [x] **3.6 Углублённая верификация RS4–RS7 (построчно)** ✅ *(2026-06-02 — 2026-06-04, dev FishEye, `@ipgCh=15`)*
  - [x] 3.6.1 **`spMstrg_2605` (`@saveToTables=1`) vs `spMstrg_2408_SaveToTables`**  
    Метод: снимок таблиц после legacy → `EXEC spMstrg_2605` → `EXCEPT` по `SELECT *` в обе стороны для `ResultSet4..7`.  
    **Исполнено:**
    - `MounthEndDate='2025-07-31'`: RS4=744, RS5=32, RS6=721, RS7=32 — **0 расхождений** ✅
    - `MounthEndDate='2024-08-31'`: RS4=0, RS5=1, RS6=0, RS7=1 — **0 расхождений** ✅
  - [x] 3.6.2 **`spMstrg_2605` (`@saveToTables=0`) vs `spMstrg_2408` (SELECT)**  
    Метод: захват всех рекордсетов (Python/pymssql), сопоставление по **именам столбцов**, сравнение multiset значений.  
    Соответствие номеров: `_2408` RS3→RS4, RS4→RS5, RS5→RS6, RS6→RS7 (у `_2605` дополнительно RS1 полный дамп 398 кол. и RS2–RS3 part_1/part_2).  
    **Исполнено (`2025-07-31`):**
    | Пара | Строк | Данные (multiset) | Порядок строк | Столбцы |
    |------|-------|-------------------|---------------|---------|
    | RS4 Rep01 | 744 | совпадают | **может отличаться** | 44, имена совпадают |
    | RS5 Rep02 | 32 | совпадают | совпадает | 44 |
    | RS6 Src01 | 721 | совпадают (50 общих полей) | может отличаться | см. 3.6.3 |
    | RS7 Src02 | 32 | совпадают | совпадает | 51 |
    - `MounthEndDate='2024-08-31'`: RS4–RS7 — **полное совпадение** (включая порядок) ✅
  - [x] 3.6.3 **Особенность RS6 (Src01)** — не регрессия `_2605`  
    В **SELECT** `spMstrg_2408` (6-й рекордсет): в начале `ogNm`, `branchName`, `cstAgPnCode`, **`cstapCstName`**, …  
    В **`spMstrg_2605`** и в таблице `ResultSet6` после `SaveToTables`: `ipgSh`, **`limSort`**, `ogNm`, `branchName`, … (без `cstapCstName` в наборе).  
    По **50 общим** столбцам данные **идентичны**; расхождение только в составе/порядке метаданных SELECT `_2408` vs INSERT/SELECT `_2605` (наследие `_2408`).  
    **Исполнено:** зафиксировано для Access: при чтении ADODB ориентироваться на **имена полей**, не на порядок; `limSort` доступен в `_2605`, в старом SELECT `_2408` — нет ✅

---

### Этап 4 — Обновление VBA (`Form_ipgChMin.cls`) ✅ *(инструкция подготовлена)*

> Изменения выполняются **вручную** в MS Access. Инструкция включена в `docs/deployment/db-upgrade-spMstrg-2605.md`.

- [x] **4.1 Подготовка инструкции для ручного выполнения** ✅
  - [x] 4.1.1 Описать добавление `cbxIpgSt` (ComboBox) на форму `Form_ipgChMin`:
    - Источник строк:
      ```sql
      SELECT '' AS cst_type, '(все стройки)' AS nm
      UNION SELECT DISTINCT cst_type, cst_type FROM ags.[importIpgSt_26-0320]
      ORDER BY 1
      ```
    - Свойства: `BoundColumn = 1`, `ColumnCount = 2`, `ColumnWidths = "0cm;4cm"`, `DefaultValue = ""`  
    **Исполнено:** раздел 6.1 документа `db-upgrade-spMstrg-2605.md`.
  - [x] 4.1.2 Описать изменения в `btnMasteringPercent_2408_Click()`:
    - `.CommandText = "ags.spMstrg_2605"`
    - Добавить `.Parameters.Append` для `@ipgSt` и `@saveToTables = 0`
    - Порядок параметров: `@ipgCh`, `@MounthEndDate`, `@ipgSt`, `@saveToTables`  
    **Исполнено:** раздел 6.2 документа — полный diff БЫЛО/СТАЛО с VBA-кодом. `@ipgSt` передаётся как `Null` при пустом `cbxIpgSt`, `@saveToTables = adBoolean False`.
  - [x] 4.1.3 Включить инструкцию в раздел «Изменения MS Access» документа `db-upgrade-spMstrg-2605.md`  
    **Исполнено:** документ `docs/deployment/db-upgrade-spMstrg-2605.md` создан (разделы 1–8).

- [ ] **4.2 Тестирование в MS Access** *(после ручного применения — выполняется вручную)*
  - [ ] 4.2.1 Кнопка с `cbxIpgSt = "(все стройки)"` → результат как у старого `spMstrg_2408`
  - [ ] 4.2.2 Кнопка с `cbxIpgSt = '12ОПР'` → результат как у `spMstrg_2408_ipgSt`
  - [ ] 4.2.3 Проверить все 6 групп

---

### Этап 5 — Обновление FEMSQ (Java / JasperReports) ✅

- [x] **5.1 Найти место вызова `spMstrg_2408_SaveToTables` в коде Java** ✅
  - [x] 5.1.1 Поиск по кодовой базе `code/femsq-backend`  
    **Исполнено:** Вызов `spMstrg_2408_SaveToTables` **не находится в Java-коде**.  
    Архитектура: `ReportGenerationService.java` читает данные напрямую из таблиц  
    `ags.spMstrg_2408_ResultSet1..7` через JasperReports + JRXML-шаблон.  
    Таблицы заполняются **внешним shell-скриптом** `code/scripts/execute_spMstrg_2408.sh`.  
    JRXML (`mstrgAg_23_Branch_q2m_2408_25.jrxml`) и Java-код не требуют изменений.

- [x] **5.2 Изменить вызов на `spMstrg_2605` с `@saveToTables = 1`** ✅
  - [x] 5.2.1 Создать `code/scripts/execute_spMstrg_2605.sh` — замена `execute_spMstrg_2408.sh`:  
    - Вызывает `EXEC ags.spMstrg_2605 @ipgCh=..., @MounthEndDate=..., @ipgSt=NULL, @saveToTables=1`  
    - Использует `docker exec femsq-mssql sqlcmd` (sqlcmd только в контейнере)  
    - Переменная `IPGST=""` — NULL = все стройки  
    - После выполнения: COUNT по всем 7 ResultSet-таблицам  
    - Обновлён `code/scripts/README.md`  
    **Протестировано:** RS1=RS2=RS3=12693, RS4=744, RS5=32, RS6=721, RS7=32 — совпадает с эталоном ✅
  - [x] 5.2.2 Проверить таймаут  
    **Исполнено:** Время выполнения ~24 сек (dev-Docker), TIMEOUT=600 сек — достаточно ✅

- [ ] **5.3 Проверить работу JasperReports** *(выполняется вручную после деплоя)*
  - [ ] 5.3.1 Запустить отчёт `mstrgAg_23_Branch_q2m_2408_25`, убедиться в корректности данных

---

### Этап 6 — Удаление устаревших объектов ✅ *(секция A выполнена)*

- [x] **6.1 Финальная верификация перед удалением** ✅
  - [x] 6.1.1 Все тесты этапов 3–5 пройдены без замечаний ✅ *(дополнено углублённой верификацией п. 3.6, 2026-06-04)*
  - [x] 6.1.2 На dev FishEye `_2605`-объекты подтверждены ✅  
    *(На продуктиве — применяется администратором отдельно)*

- [x] **6.2 Подготовка скрипта удаления** ✅
  - [x] 6.2.1 Оформить `06_DROP_obsolete_2408.sql` — создан с двумя секциями:  
    **Секция A** (выполняется сейчас, промежуточные `_ipgSt`-варианты):
    - `DROP PROCEDURE IF EXISTS ags.spMstrg_2408_SaveToTables_ipgSt`
    - `DROP PROCEDURE IF EXISTS ags.spMstrg_2408_ipgSt`
    - `DROP FUNCTION IF EXISTS ags.fnIpgChRsltCstUtlPercentBrn_2408_ipgSt`
    - `DROP FUNCTION IF EXISTS ags.fnIpgChRsltCstUtl2_2408_ipgSt`
    
    **Секция B** (закомментирована — только после полного перехода на `_2605` на продуктиве):
    - `DROP PROCEDURE IF EXISTS ags.spMstrg_2408_SaveToTables`
    - `DROP PROCEDURE IF EXISTS ags.spMstrg_2408`
    - `DROP FUNCTION IF EXISTS ags.fnIpgChRsltCstUtlPercentBrn_2408`
    - `DROP FUNCTION IF EXISTS ags.fnIpgChRsltCstUtl2_2408`
    
    **Исполнено:** На dev FishEye `_ipgSt`-объектов не существовало → IF EXISTS = no-op.
  - [x] 6.2.2 Выполнить на FishEye; убедиться, что `_2605` продолжает работать  
    **Исполнено:** 3 объекта `_2605` подтверждены после выполнения скрипта ✅

---

### Этап 7 — Сборка и приёмка пакета для продуктивного сервера ✅

- [x] **7.1 Финальная подготовка артефактов**
  - [x] 7.1.1 Проверить идемпотентность 01–03: выполнить повторно — ошибок нет  
    **Исполнено:** 01/02/03 повторно — `OK: создана/обновлена` / `создана успешно!` ✅
  - [x] 7.1.2 Проверить откат: выполнить `05_ROLLBACK.sql` → затем 01–03 → `04_VERIFY_after.sql`  
    **Исполнено:** откат удалил 3 объекта `_2605`; повторное применение восстановило их без ошибок ✅
  - [x] 7.1.3 Убедиться, что в `04_VERIFY_after.sql` все ожидаемые значения прописаны в комментариях  
    **Исполнено:** добавлены подтверждённые значения: RS1..7=12693/12693/12693/0/1/0/1 (full), 604/604/604/0/1/0/1 (12ОПР); время выполнения ✅

- [x] **7.2 Оформление документа порядка работ**
  - [x] 7.2.1 Создать `docs/deployment/db-upgrade-spMstrg-2605.md` по структуре  
    **Исполнено в Этапе 4:** документ создан полностью (8 разделов) ✅
  - [x] 7.2.2 Заполнить таблицу приёмки ожидаемыми значениями из `04_VERIFY_after.sql`  
    **Исполнено:** раздел 8.2 заполнен конкретными значениями RS1..7; версия 1.1 ✅

- [x] **7.3 Финальная приёмка пакета на FishEye**
  - [x] 7.3.1 Выполнить `00_VERIFY_before.sql` — состояние «до»  
    **Исполнено:** создан `00_VERIFY_before.sql`; все 4 объекта `_2408` ОК, таблицы 7/7, `_2605` отсутствуют ✅
  - [x] 7.3.2 Применить 01 → 02 → 03 в порядке номеров  
    **Исполнено:** все три скрипта отработали успешно ✅
  - [x] 7.3.3 Выполнить `04_VERIFY_after.sql` — сравнить с ожиданиями  
    **Исполнено:** объекты созданы; fn2 90=90, fnPBrn 398=398; spMstrg_2605 все режимы ОК ✅
  - [x] 7.3.4 Выполнить `05_ROLLBACK.sql` → убедиться, что `_2605` удалены, `_2408` работают  
    **Исполнено:** откат — 3 DROP; `fnIpgChRsltCstUtl2_2408` вернул 14210 строк ✅
  - [x] 7.3.5 Повторно применить 01–03 (проверка чистоты отката и повторного применения)  
    **Исполнено:** повторное применение прошло без ошибок; 3 объекта `_2605` созданы ✅

---

### Этап 8 — Документирование ✅

- [x] **8.1** Обновить `docs/solutions/spMstrg_2408_execution.md` — добавить раздел о `spMstrg_2605`  
  **Исполнено:** добавлен раздел «Обновление 2026-05» (~100 строк): новые объекты, сигнатура, режимы, таблица RS-значений, время выполнения, откат, таблица артефактов ✅
- [x] **8.2** Обновить `docs/project/project-docs.json` — секция `reports.implemented`, источник данных  
  **Исполнено:** запись `report:mstrgAg_23_Branch_q2m_2408_25` обновлена: `dataSource.procedure → spMstrg_2605`, добавлен `@ipgSt` в `parameters`, версия `1.1.0`, новые `relatedDocuments` ✅
- [x] **8.3** Создать резюме чата `docs/development/notes/chats/chat-resume/chat-resume-26-0508-spMstrg-2605.md`  
  **Исполнено:** резюме создано (~80 строк): итог, ключевые изменения, проблемы и решения, таблица результатов тестирования ✅
- [x] **8.4** Сделать запись в `docs/journal/project-journal.json`  
  **Исполнено:** добавлена сессия `chat-2026-05-08-001` с 5 log-записями (этапы 1–7); `metadata.lastUpdated → 2026-05-16` ✅

---

## Контрольные точки

| Точка | Условие готовности |
|-------|--------------------|
| К-0 | Эталонные данные зафиксированы; базовые `_2408`-объекты и `importIpgSt_26-0320` подтверждены; `00_VERIFY_before.sql` сформирован |
| К-1 | `fnIpgChRsltCstUtl2_2605` создана, тест пройден; `01_*.sql` и фрагмент `04_VERIFY_after.sql` готовы |
| К-2 | `fnIpgChRsltCstUtlPercentBrn_2605` создана; COUNT и суммы совпадают с эталоном; `02_*.sql` готов |
| К-3 | `spMstrg_2605` создана; оба режима протестированы (COUNT + **построчно RS4–7**, п. 3.6); `03_*.sql`, `04_VERIFY_after.sql`, `05_ROLLBACK.sql` готовы |
| К-4 | VBA обновлён вручную; форма Access работает во всех вариантах |
| К-5 | FEMSQ переключён на `spMstrg_2605`; отчёт JasperReports выдаёт корректные данные |
| К-6 | `06_DROP_obsolete_2408.sql` готов и проверен на FishEye |
| К-7 | Пакет 00–05 прошёл полный прогон (применение + откат + повтор) на FishEye; `db-upgrade-spMstrg-2605.md` заполнен |

---

## Справочная информация

### Машина разработки
- **Среда:** `nb-win` (WSL2, Ubuntu 24.04 на Windows 11)
- **БД:** Docker-контейнер `femsq-mssql`, `localhost:1433`, БД `FishEye`
- **SQL Server:** 2022 (RTM-CU23), `DB_ID('FishEye') = 5`
- **DBHub:** `.cursor/dbhub/node_modules/@bytebase/dbhub/dist/index.js` (таймаут 15 сек)
- **sqlcmd:** внутри контейнера `/opt/mssql-tools18/bin/sqlcmd -C`

### Ключевые параметры для тестирования
- `@ipgCh = 15` (цепь инвестпрограмм)
- `@MounthEndDate` — последний день актуального месяца
- `@ipgSt = NULL` — все стройки (эталон: RS1–3: 12693, RS4: 744, RS5: 32, RS6: 721, RS7: 32)
- `@ipgSt = '12ОПР'` — группа для сравнения с `_ipgSt`-вариантами

### Соответствие рекордсетов (`spMstrg_2408` → `spMstrg_2605`, `@ipgSt=NULL`)

| `spMstrg_2408` (6 RS, Access) | `spMstrg_2605` save=0 (7 RS) | Таблица save=1 | Строк (2025-07-31) |
|------------------------------|------------------------------|----------------|---------------------|
| RS1 part_1 (179 col) | RS2 | ResultSet2 | 12693 |
| RS2 part_2 (220 col) | RS3 | ResultSet3 | 12693 |
| RS3 Rep01 | RS4 | ResultSet4 | 744 |
| RS4 Rep02 | RS5 | ResultSet5 | 32 |
| RS5 Src01 | RS6 | ResultSet6 | 721 |
| RS6 Src02 | RS7 | ResultSet7 | 32 |
| — | RS1 полный (398 col) | ResultSet1 | 12693 |

> Построчная верификация RS4–RS7: **п. 3.6** (2026-06-04). Для «разреженного» месяца `2024-08-31`: RS4–7 = 0/1/0/1.

### Расположение артефактов
- **Пакет для продуктива (SQL Server 2016+):** `docs/development/notes/sql/26-0508/` (файлы 00–06)
- **Пакет для продуктива (SQL Server 2012 SP4+):** `docs/development/notes/sql/26-0508/MSSQL2012/` (файлы 00–06)
- **Документ порядка работ:** `docs/deployment/db-upgrade-spMstrg-2605.md`
- **Чеклист дня деплоя:** `docs/deployment/db-upgrade-spMstrg-2605-deploy-day-checklist.md`
- **Эталонные `_2408` скрипты:** `docs/development/notes/sql/26-0416/`
- **VBA-исходник:** `docs/project/proposals/vba-analysis/VBA-Code-Export/Form-Modules/Form_ipgChMin.cls`

---

## Вскрытые архитектурные проблемы *(2026-06-04, вне исходного объёма чата)*

> Анализ проведён в сессии 2026-06-04 в рамках подготовки к `spMstrg_2606`.
> Разработка `spMstrg_2606` ведётся в **отдельном чате** с планом
> `docs/development/notes/chats/chat-plan/chat-plan-26-0604-spMstrg-2606.md`.

### Проблема A — Некорректная фильтрация через `importIpgSt_26-0320`

**Суть:** в `fnIpgChRsltCstUtl2_2605` фильтр по группе строек (`@ipgSt`) использует таблицу `importIpgSt_26-0320`, привязанную к `cstAgPnCode` (текстовый код пункта ИПГ) **без учёта актуальной на дату инвестпрограммы**.

**Последствие:** для дат, покрываемых разными `ipg` в пределах цепи, фильтр применяет состав группы из всех `ipg` одновременно, а не только из актуальной на конкретную дату. Если состав группы меняется между корректировками ИПГ — строки включаются или исключаются неверно.

**Подтверждение (цепь 15, `ipg=25` → `ipg=26`):** для `12ОПР` — 1 строка присутствует в `ipg=26` но не в `ipg=25`; 43 строки меняют узел `stIpg` между `ipg=25` и `ipg=26`.

**Правильное решение:** фильтрация через `ipgStPn` (привязка пунктов ИПГ к узлам DAG структуры ИПГ) с учётом актуальной `ipg` на дату — реализуется в `spMstrg_2606`.

**Ограничение `spMstrg_2605`:** фильтр через `importIpgSt` остаётся корректным при условии что состав группы стабилен между корректировками ИПГ в цепи. Для `12ОПР` в цепи 15 расхождение на практике невелико (1 строка), но не гарантировано на будущих данных.

### Проблема Б — Ошибка схемы данных: `ipgStr/ipgEnd` как атрибут `ipg`, а не `ipgChRl`

**Суть:** даты начала/конца актуальности инвестпрограммы (`ipg.ipgStr`, `ipg.ipgEnd`) являются атрибутами сущности `ipg`, а не атрибутами связи «цепь — инвестпрограмма» (`ipgChRl`). Это означает, что одна и та же `ipg` не может иметь разные периоды актуальности в разных цепях.

**Проявление на цепи 15:** `ipg=25` (Одобренная) используется в цепях 14 и 15. Для цепи 15 в июле 2025 введена `ipg=26` (Письмо Д644). При `ipg=25.ipgEnd=NULL` и `ipg=26.ipgStr=2025-07-16` оба `ipg` покрывают месяцы 7–12 одновременно — расчёт задвоен. Исправление `ipg=25.ipgEnd` сломает цепь 14.

**Дополнительное ограничение `_2605`:** фильтр актуальности работает на уровне **номера месяца** (`MONTH(ipgStr)`), не дня. Даже при корректных `ipgEnd` переход в середине месяца (как здесь: 16 июля) даёт задвоение по всему месяцу.

**Правильное решение:** новая таблица `ags.ipgChRlV` — даты актуальности как атрибут связи «цепь — ИПГ»; `ipgcrvEnd` вычисляется как `DATEADD(day,-1, следующий ipgcrvStr в цепи)`, исключая необходимость ручного ввода и гарантируя отсутствие перекрытий. Реализуется в `spMstrg_2606`.

**Статус `spMstrg_2605`:** известное ограничение — задвоение месяцев 7–12 для цепи 15 при `@ipgSt=NULL` (без фильтра) проявляется в удвоении строк по каждой стройке за эти месяцы. При `@ipgSt='12ОПР'` эффект минимален (1 лишняя строка). `ipg.ipgStr/ipgEnd` не исправляются — изменение затронет цепь 14.
