...
**Проект:** FEMSQ  
**Версия плана:** 0.17.0 (S19: перегрузка однозначных карточек без doc_base)  
...
|----|----------|--------|------|-------|---------------|
| P1 | Квартальная подготовка «Исходящих сведений» | оператор СУДЗ (Access) / специалисты по долгам / согласующие разделов / получатель итога | общий свод + `export_{счётГК}` | «Исходящие сведения» после согласования | высокий (ядро) |

**Суть P1 (S2+S3):** работа **ежеквартальная**. Объект года Y — только долги, просроченные на **31.12.(Y−1)**. Через Access в FishEye.ags грузятся свод и `export_*` (привязка к `cst`/`cstAgPn`); выгружается `ags_Yr_DbtChangesRslt` для сбора мероприятий; затем готовится и согласуется итог. Новая просрочка внутри года Y **не** ведётся и **не** попадает в исходящие сведения. Свод на 31.12.Y задаёт портфель на год Y+1.
...
|--------|---------------------|
| `2025-12` | Общий свод ДЗ на 31.12.2025 + выгрузка `ags_Yr_DbtChangesRslt_*` |
| `2026_03` | Общий свод на 31.03.2026; `export_{счётГК}_*`; `ags_Yr_DbtChangesRslt_*`; кейс `26-0526_Головинова` (xlsx + PDF претензий) |
...
| **Исходящие сведения** | *(пример пока нет)* | Итог после 1.2.*; готовится из БД через Access |
...
**Связь с уже известным в FEMSQ:** имена `ags_Yr_DbtChangesRslt`, `export_*`, счета ГК и поля «№ контрагента / договор / документ основания» стыкуются с контуром `invDbt` / `cn_inv_dbt` / импортами `importDbt_*` — точный маппинг ещё не зафиксирован (см. В5).
...
| В9 | Пример **исходящего** Excel для подразделения? | ☐ открыт |
| В10 | «Перенос срока погашения» — как отражается в следующем своде? | ☐ открыт |
| В11 | Где фиксируются **мероприятия**? | ☐ гипотеза: `cnInvCmm*` ([04](../../domain/sudz/04-data-model.md) §2.6) |
| В12 | Какие формы/запросы Access на шагах 1.1.1.* / 1.1.3 / 1.2.*? | ☐ открыт |
