...
| **C.10.1** | UI **901**: excelToTbl → фазы A–E (G0…G5) | G2–G4 open=0; DV≈Tbl (±док. искл.) | ✅ **2026-09-14** G2–G4 open=0; UI чекбокс 8 `invDbtDbtEnsure` + 9 `dbtValueLoad` (apply, flTbl=false); Tbl=1774, DV≈1775; L*/Split UI не переигрывали |
| **C.10.2** | Smoke P3 @901 | **9/9** сумм в `DbtValue@901` (~592,9M) | ✅ 2026-09-09 (7×КС-51: снят мост 2115; 105449: Create ciud 3786 → slot 13043) |
| **C.10.3** | Gate QIV `excel_only` / T1 (и T2 без P3-gap) | vs `26-0212` или база `26-0505` | ✅ T1@901 PASS (2026-09-14); зерно свода подтверждено S77.8 @902 (2×18000) |
| **C.10.4** | Reset/подготовка **902**; UI **902** теми же gate'ами | G2–G4 PASS | ✅ **2026-09-15** G0–G2 PASS (лог); G3/G4 open=**0**; G5 DV=**1746**/Tbl=1746; без однозн.=0; **S77.7b** B0–B4 |
| **C.10.5** | Export Rslt (yr 2026, asOf ≥902) ↔ **`26-0505`** | P3: **8/9** в QI; **105449** — documented exception | ✅ **2026-09-15** ∑ base/QI **Δ=0**; зерно **2×18000** (Access 1×36000); artifact `stage2_qi_s778_902_26-0915.json`; soft-key drift Access-формат (не блокер зерна) |
| **C.10.6** | **903** / QII compact: резолв даты договора + UI gate'ы | см. [C.10.6](#c106--qii-903-компактный-свод-резолв-даты-стороны-2026-09-15) | ✅ D0–D4 2026-09-16 |

###### C.10.6 — QII 903 компактный свод / резолв даты стороны (2026-09-15)

**Контекст:** reset+excelToTbl@903 → Tbl=**1803**, все `cidutCnDate=NULL` (компактные листы без «Дата договора»). G1 CnNotLoad PASS; G2 ExistCtpt **220** — ложные FAIL: Excel→sentinel `1900-01-01`, в БД сторона с реальной датой. FileSh shortlist: 606012/606022/761010/762210/767501/767502.

**Политика владельца (утверждена 2026-09-15):** при **пустой** дате в Excel/Tbl — не `MAX(csoCnDate)` как основной выбор.

| # | Правило |
|---|---------|
| 1 | Excel/Tbl дата есть → алгоритм Access/FEMSQ без изменений |
| 2 | Даты нет → `csoCnDate` / сторона из **Value (var) max prior upl** той же пары БУиРГ+№ (`yr_upl_p`, upl&lt;curr) |
| 3 | Нет Value → дата из **Tbl** того же prior upl |
| 4 | Нет prior → единственная датированная сторона **или** уже существующая null-сторона (Tbl остаётся NULL) |
| 5 | Сторон org+№ нет → **create** `cn_s_org` с пустой датой (только при `flLoad`; `countCnName=1`) |
| 6 | `MAX(csoCnDate)` — не основной выбор (опц. лог/fallback later) |

**Куда применять:** обогащение `cidutCnDate` в Tbl (и create null) **до** ExistCtpt и всей EIA (AccSmpl / `invDbtVarEnsure`), иначе зелёный G2 и дыры дальше.

**Запреты:** create null при уже существующих датированных сторонах; full G3–G5 до PASS ExistCtpt (кроме явного хвоста); тихий MAX.

| # | Мероприятие | Содержание | Критерий | Статус |
|---|-------------|------------|----------|--------|
| **D0** | Политика в плане | правила 1–6 + запреты | текст согласован | ✅ 2026-09-15 |
| **D1** | Код: resolve null dates | `resolveDbtUplNullCnDates` (Value→Tbl→unique side; create null при `flLoad`); prelude в `CnExistCtptNotLoad`; JAR **0.1.0.284** | compile + лог счётчиков | ✅ 2026-09-15 |
| **D2** | Spike UAT @903 | Префикс 1–3, `flLoad=true`, flTbl=false | ExistCtpt **0**; resolve: value=**1757**, side=**44**, nullSide=**2**, created=**0**, unresolved=**0**; G0/G1 PASS | ✅ 2026-09-15 |
| **D3** | UI G3–G5 @903 | По runbook после D2 PASS | G3/G4 open=0; DV≈Tbl | ✅ GraphQL 2026-09-15; **D3-UI** ✅ 2026-09-16 browser @cloud.ru; **повтор nb-win** ✅ 2026-09-16: G0–G5; DV=Tbl=**1803**; P1=**364** |
| **D4** | Rslt asOf903 / Excel enrich | вне воронки; мост QIV+QI+QII | export + ∑ | ✅ **2026-09-16 nb-win:** воронка **902** UI G0–G5 → DV=**1746**; re-export `yr=901 asOf=903` — **QIV/QI/QII PASS** (vs `26-0505` Δ=0; QII≡Excel shortlist Δ=0). Арт.: [`…d4_post902_26-0916.xlsx`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/ags_Yr_DbtChangesRslt_901_asOf903_d4_post902_26-0916.xlsx), [`stage2_qii_d4_903_post902_26-0916.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/stage2_qii_d4_903_post902_26-0916.json). P1@902=193 / @903=364 — backlog |

**Связь:** сегм. 17 (дата в тройке) — исключение только для compact/null Excel; S65b/S65c (create null / не эвристика при коллизиях номера); shortlist QII / business Excel enrich — D4.
...
**Запреты:** full apply без gate'ов (C.9); B-seed; сравнивать Rslt до G2–G4 PASS. **S76-A** не смешивать с C.10 до FAIL после закрытых gate'ов. Live-split 85166 / ALTER UNIQUE и Consistency — **не в C.10**; это трек **S77** ([08 §3.3](../../sql/26-0908-sudz-split-merge-sandbox/08_CMM_AND_RSLT.md)).

**2026-09-08:** песочница `test_sudz_sm` — канон Rslt **A** и **C**. C.10.3/C.10.5 не открывать, пока S77 не отдаст Rslt с зерном свода. G4 2807/2808 — фикстура, не `deferred` «сумма сошлась».

##### S77 — split/merge ядро + воронка + Rslt A/C (DEV) (2026-09-08)

**Задача:** 0069. **Чат реализации:** `chat-2026-09-08-002`. Песочница `test_sudz_sm` остаётся `chat-2026-09-08-001`. Отдельный файл плана не заводим.

**Цель:** полноценный split/merge в схеме **`sudz` (DEV)** и в составе воронки, чтобы FEMSQ отдал Rslt, который можно положить рядом с `ags_Yr_DbtChangesRslt_26-0505.xlsx`: зерно колонок QIV/QI = зерно свода **того** среза; комментарии по зерну своей группы; «погашено» на каноне `Dbt`, не по долям.

**Не цель:** узкий срез «только 85166 в Excel»; «Rslt без split»; PASS при 1×36000 vs свод 2×18000; `MSSQL2012/` / прод; full `runSudzDbtUplFunnel` вместо UI.

**Критерий готовности:** файл FEMSQ сопоставим с `26-0505` по зерну (не только по ∑). Первая живая точка — `iKey=85166` / ciud **2807+2808** @901, но это **класс** `multi`, не разовый SQL.

**Стоп-линия C.10:** не G5→Rslt на текущей модели. 2807/2808 остаются фикстурой (`open`). Закрытые G0–G3 @901 не откатываем.

**Канон формы Rslt** (уже зафиксирован, не пересматриваем здесь): [08 §3](../../sql/26-0908-sudz-split-merge-sandbox/08_CMM_AND_RSLT.md) — полосы **A** и **C**.

| Колонки | Зерно (08 §3.1; процесс [03 §1.1.2](../../domain/sudz/03-processes.md)) |
|---------|-----------------|
...
| «Старые» cmm (`yr_CmmGr`) | `DbtValue` **предшествующего** среза группы (пример: **QIV**, когда крайняя полоса фактов — QI). На рассылке специалистам новых ещё нет |
| `*_new` | **два этапа одного файла** `ags_Yr_DbtChangesRslt_*.xlsx` (ниже) |

Файл Rslt участвует в **двух** процессах ([03 §1.1.1.3–1.1.2.2](../../domain/sudz/03-processes.md); код: `SudzRsltExcelExporter.exportRsltSborn` / `exportRsltPovtor`, REST `rslt-sborn.xlsx` / `rslt-povtor.xlsx` + импорт возврата в `yr_CmmGr_New`):

1. **Рассылка (Rslt сбор, 1.1.1.3):** «старые» ← `yr_CmmGr` = комментарии предшествующего среза (QIV); колонки `*_new` **пустые**; `yr_CmmGr_New` не участвует. Специалисты читают старые и вписывают новые **для текущей** крайней upl (QI).
2. **Возврат → БД (1.1.2.1) и контроль (Rslt повтор, 1.1.2.2):** `*_new` из вернувшегося Excel заливаются в `yr_CmmGr_New`. Повтор читает «старые» из `yr_CmmGr` и `*_new` из New. Зерно `*_new` = `DbtValue` **текущего** раунда (QI). Дальше D644/свод читают уже `yr_CmmGr` (веха 1.1.3).

Число строк полосы = max(зерно факта крайней upl, зерно старых cmm, зерно `*_new` если они заполнены). Колонка с зерном 1 сливается. **A** — крайний срез уже split; **C** — крайний снова целый.

###### S77.1 — замена инвариантов (бумага; ALTER после согласия)

Источник: [08 §3](../../sql/26-0908-sudz-split-merge-sandbox/08_CMM_AND_RSLT.md) + план чата 2026-09-08 (не узкий срез). Черновик DDL: [26-0908-sudz-split-merge-dev](../../sql/26-0908-sudz-split-merge-dev/) — **не применять**.

**БД (`sudz` DEV):**

| # | Было (живой `sudz` / S16 / Consistency) | Станет |
|---|----------------------------------------|--------|
| I1 | `UNIQUE(iddInv, iddDbt)` — один `Dbt` на СФ не более одного слота | N слотов одного `Dbt` на одном СФ (форма A). `UNIQUE(iddInvDbt)` **жив** (слот → один канон) |
| I2 | Consistency **п.5**: одна Value на `Dbt×upl` | N Value на `Dbt×upl`, **по одной на слот**. `UNIQUE(dvInvDbt, dvUpl)` **жив** |
| I3 | Consistency **п.4**: sibling с одинаковым `dvTtl` запрещён (в т.ч. 2×18000) | одинаковые доли **разрешены**, если слоты — части **одного** канона; same-ttl у **разных** `Dbt` на одном СФ по-прежнему запрещён (P1-дубль) |
| I4 | cmm / `cnInvGr` якорь = `Dbt` (`*Dbt` рядом с `*InvAccnt`, D3′) | якорь раунда = `DbtValue`; nullable `*Dv` рядом с D3′ `*Dbt`; Access-мост `*InvAccnt` не трогаем. D3′ **не** отменяем |
| I5 | «погашено» / cutover **D4**: одна строка Rslt на `dbtKey` | «погашено» на каноне (I5-формула); **число строк факта среза** = число Value этой upl, не 1×`Dbt`. D4 остаётся верным для L* (один Value / upl) |

**Оставляем без изменений:** Consistency п.1–3 (cnInv, слот=СФ var, мост var); `UNIQUE(idInv, idNum)` на `invDbt`; `trg_invDbtDbt_InvMatchesSlot`; L* / P1 как «новый слот + тот же `Dbt`» (D2).

**Не триггер (приложение / Split-мутация):** сумма долей на upl = Value канона до split; Link 18000 на слот 36000 — отказ; L* «на upl жива одна доля» — правило calm/воронки, не п.5 (п.5 мешает и split, и merge-across-inv; в песочнице merge моста уже обходил п.5).

**Согласовать явно (запреты), затем DDL:**

1. Снять `UX_invDbtDbt_InvDbt` на **`sudz`**. `test_sudz` (82/85) **не** трогать, пока отдельно не решим.  
2. п.4: same-ttl OK **только** при общем `iddDbt`.  
3. п.5 **удалить** (не «ослабить до того же inv» — это сломает merge 113→112).  
4. `UNIQUE(dvInvDbt, dvUpl)` не снимать.  
5. D3″ (`*Dv`) — **тот же трек**, но отдельный скрипт; можно после Split, до экспортёра A/C.  
6. Не `MSSQL2012/`, не продуктив, не ALTER до пункта «согласовано».

###### Порядок трека (Rslt бизнесу — только в конце)

| Шаг | Содержание | Статус |
|-----|------------|--------|
| **S77.1** | Текст инвариантов + согласие на запреты | ✅ 2026-09-08 (зерно cmm: предшествующий срез / два этапа Rslt) |
| **S77.2** | DDL DEV (`sudz` only): DROP UX + переписать Consistency п.4, убрать п.5 | ✅ DEV 2026-09-08; I4 `02` *Dv + backfill `04` ✅ 2026-09-09; `test_sudz` не тронут |
| **S77.3** | Мутации GraphQL **Split** / **Merge** (те же, что вызовут экран и воронка) | ✅ 2026-09-08; UI КСДД не вешался |
...
| **S77.7** | Остаток G5 901 → построчный T1@901 (реестр ошибок эталона) → UI 902 → S77.8 | ✅ G5@901; T1@901 PASS; UI **902** G0–G5 ✅ 2026-09-15 |
| **S77.7b** | Dry-лог Calm Create на `invDbtLoad` + закрытие дыры «без однозн. Excel→slot» @902 | ✅ **B0–B4**; **B5** ☐ later |
| **S77.8** | Rslt vs `26-0505` → снять блокер C.10.3 / C.10.5 | ✅ **2026-09-15** ∑ base/QI Δ=0; зерно 2×18000; quality [`stage2_qi_quality_902_26-0915.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/stage2_qi_quality_902_26-0915.json); **E-RowGrain-28469** → S77.9 |
| **S77.9** | Непрерывность слотов (`первая`/`вторая` @28469): M1–M5 ✅ | ✅ **2026-09-15** |

###### S77.7 — политика 2026-09-14 (рамочная работоспособность vs полный операторский UAT)

**Цель сейчас:** довести **901** до **построчного PASS T1** (ниже), затем **902** gate'ы → сверка с `26-0505`.

**PASS T1 @901 (до 902), согласовано 2026-09-14:**

- Сверка FEMSQ Rslt QIV (`yr=900`, `asOfUpl=901`) ↔ Access QIV из **`26-0212`** (полный портфель; не узкая 1-я полоса `26-0505`).
- Область: **пересечение с Tbl@901** (excel_only).
- Ключ строки (черновик): `(account, invNorm, cnNorm, ttl±0.01, overd±0.01)`.
- **Реестр установленных ошибок эталона** (whitelist): казусы Access, подтверждённые сводом/Tbl; FEMSQ **не** подгоняется под них; вне реестра only-in-* / mismatch → **FAIL**.
- ∑ портфеля — индикатор, не замена построчному gate.
- Split: зерно = **свод** (напр. 2×18000), не схлопнутый Access (1×36000) — в реестр.

**Отложить (backlog после S77.8):** повторный UI-UAT казусов **L\*** и **«Разделение долгов»** через браузер. Порядок слотов/Value по ним в DEV — следствие разработки (S77.6 Split 85166, правки, seed), а не «эксплуатационного» разбора на чекбоксах 8–9. Чекбокс **8** = `invDbtDbtEnsure` (автомост), **9** = `dbtValueLoad` (Value/P1) — **не** место ручного Split/L*; ручной контур — **G4/КСДД** (чекбокс 7) и экран **S78** «Долг (канон)».

**Делать сейчас:**
1. ~~**901:** формально закрыть G5 (`dbtValueLoad`)~~ ✅ 2026-09-14.
2. ~~QIV-smoke ∑ после удаления cn 2536~~ ✅ 2026-09-14 (Δttl vs `26-0212`/`26-0505` = 0; −7454 к утреннему GEN).
3. ~~**Inventory** GEN↔Access∩Tbl~~ ✅ (ложный empty-inv в 1239; recheck [`…-1300.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/stage2_qiv_t1_inventory_901_26-0914-1300.json)).
4. ~~**Реестр ошибок эталона**~~ ✅ E-Split, E-Access-CnMultiNum, E-Inv-Alias-BS-SGM14-234, E-Access-Missing-86740-732.
5. ~~**Фиксы FEMSQ Rslt**~~ ✅ merge fill-down; `account_num` с последнего upl (JAR **0.1.0.271**).
6. ~~Повтор T1 → **PASS**~~ ✅ 2026-09-14 FAIL=0 вне реестра.
7. ~~**S77.7b** — dry-лог Calm Create на `invDbtLoad` + закрытие 4×`no_candidate_slot` @902~~ ✅ 2026-09-15 (B0–B4).
8. ~~**902:** UI G0…G5 после B2–B4; закрыть КСДД open~~ ✅ 2026-09-15: G0–G2 лог PASS; G3/G4 open=0; G5 DV=1746; без однозн.=0. **P1 open=193** (none/single/multi) — не G4, backlog.
9. ~~**S77.8:** Rslt ↔ `26-0505`~~ ✅ 2026-09-15: export `yr=901 asOf=902`; ∑ base_ttl/o + q1_ttl/o/pog **Δ=0**; А45-19974/2024 = **2×18000** (эталон Access 1×36000); artifact [`stage2_qi_s778_902_26-0915.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/stage2_qi_s778_902_26-0915.json).

**Запреты без изменений:** full apply без gate'ов на 902; сравнивать Rslt до G2–G4 PASS на 902; подгонять FEMSQ под строки из реестра ошибок эталона; **apply Value@902 при «без однозн. Excel→slot» > 0**.

Параллелить «воронку без экспортёра» или «экспортёр без DDL» нельзя.

**Split (смысл):** канон + слот с одной Value → N слотов / N Value на этой upl (∑ сохраняется); история предыдущих upl не режется.

**Merge (смысл):** N канонов или N долей → один `Dbt`; Value остаются на слотах (как 113→112 в песочнице); на текущей upl при слиянии долей — снова одна Value (полоса C).

**GraphQL (S77.3):** `splitSudzDbt` / `mergeSudzDbt`. **КСДД (S77.4):** советник `action=split` + кнопка «Split на доли»; `ciudKey` доли закрывает очередь (`created`). Кнопки Create/Link при split выключены.

###### S77.7b — dry-лог Calm Create на invDbtLoad / дыра Excel→slot @902 (2026-09-15)

**Контекст:** dry `dbtValueLoad`@902 (flLoad=false): tail≈1732, **«без однозн. Excel→slot» = 4**, причина `no_candidate_slot`. Value **не** писать, пока дыра не закрыта.

**Диагноз (cloud.ru / FishEye@10.7.0.1):** обрыв на **чекбоксе 7** (`invDbtLoad`), не на 9.

| cidutKey | СФ | Договор | Сумма | iKey | idvvKey |
|----------|-----|---------|-------|------|---------|
| 49482 | 1025 | 8206Д-25/ГГЭ-45876/1 | 6 332 971,16 | 92012 | 15641 |
| 49499 | 23393 | 0769Д-26/ГГЭ-56830/1 | 3 083 019,06 | 92013 | 15640 |
| 49503 | 9206 | 0257Д-26/ГГЭ-58706/1 | 15 404 998,16 | 92014 | 15639 |
| 49608 | 23978 | 16 | 344 445,47 | 92015 | 15642 |

Шаги 1–6 ок (СФ **2026-09-02**, AccSmpl тот же день, var **2026-09-04 18:45**). Слотов `invDbt` = 0, КСДД = 0 → `SHOULD_BE_CALM_CREATE`. В день var (**12** INSERT, все в Tbl@902) слотов за день = **0** — apply 7 не шёл / шёл dry. `needQueue` не ловит «4FK+var + 0 слотов» → тишина до 9-го. Сейчас `cidufFlLoad=0` у File@902.

**Правило владельца:** первым шагом — даже при **сухом** прогоне чекбокса **7** эти строки Excel должны быть **в логе** (как missing у `invDbtVarEnsure`). «Очередь пуста» ≠ «всё хорошо».

###### Мероприятия S77.7b (порядок)

| # | Мероприятие | Содержание | Критерий | Статус |
|---|-------------|------------|----------|--------|
| **B0** | Контракт лога dry-7 | Снимок до записи: **Calm Create** (`#calmOne`, слота ещё нет) + при необходимости **Calm F1** + очередь КСДД + опц. «тихая дыра» (4FK+`idvvKey`, слотов 0, не calm и не queue). Строка: договор, СФ, сумма, `cidutKey`, `iKey`, `idvvKey` | согласован с владельцем | ✅ 2026-09-15 |
| **B1** | Код: materialize workset на dry | В `runInvDbtLoadPhase` при `flLoad=false` всё равно строить `#calmOne`/`#f1One` (без INSERT); расширить результат фазы + `SudzDbtUplInvDbtLoadLog` | dry-7 пишет списки/счётчики | ✅ JAR **0.1.0.282** |
| **B2** | UAT dry-7 @902 | Только чекбокс 7 / префикс до `invDbtLoad`, «Обновлять» выкл. | в логе ≥4 Calm Create (наши cidut); опц. хвост «тихая дыра» = 0 | ✅ 2026-09-15: Calm Create=**4** (49482/49499/49503/49608); тихая дыра=нет; flLoad=false |
| **B3** | Apply только 7 | Чекбокс 7 / префикс до `invDbtLoad`, «Обновлять» вкл. (**внимание:** calm by design пишет и `DbtValue`) | слот+мост на 92012–15; dry-7 → Calm Create пуст | ✅ 2026-09-15: Auto invDbt=**4**, слоты 13052–55 + мосты на 15641/40/39/42; dry → Calm Create=**нет** |
| **B4** | Контроль 9 | Dry `dbtValueLoad` | «без однозн. Excel→slot» = **0**; затем apply 9 | ✅ 2026-09-15: dry+apply — Skip=**1746**, tail=**0**, без однозн.=**0**; DV@902=**1746**; 4 слота с Value; P1 open=**193** (очередь multi, не дыра Excel→slot) |
| **B5** | Усиление (после B1–B4) | Лог «тихая дыра»; решать отдельно: только лог vs reason `missing_slot` в КСДД | нет silent drop | ☐ later |

**Запреты:** не создавать слоты в шаге 9; не full apply Value@902 до B2–B4; не считать dry-7 «ок» при пустой КСДД без блока Calm Create.

**Связь:** блокер UI **902** G5 / C.10.4 снят (B2–B4 ✅); **S77.8** / C.10.5 ✅ 2026-09-15 (∑+зерно). Backlog: P1=193; **B5** later.

###### S77.8 — Rslt vs `26-0505` (2026-09-15)

**Export:** `GET /api/v1/sudz/rslt-sborn.xlsx?yr=901&asOfUpl=902` → срезы `2026-01-15` (base) + `2026-04-15` (QI).

**Критерии (закрыты):**

| Проверка | Результат |
|----------|-----------|
| ∑ `base_ttl` / `base_o` / `q1_ttl` / `q1_o` / `q1_pog` | **Δ=0** vs Access |
| Зерно А45-19974/2024 | FEMSQ **2×18000**; Access **1×36000** (ожидаемо; канон = свод) |
| G0–G5 @902 | PASS (КСДД/КСДСФ open=0; DV=1746) |

**Не блокер:** soft-key multiset drift (Access multi-num / alias) — как whitelist T1; **P1 open=193** (none=145, single=22, multi=26) — очередь multi-Dbt, не G4.

**Артефакты:** [`stage2_qi_s778_902_26-0915.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/stage2_qi_s778_902_26-0915.json), [`ags_Yr_DbtChangesRslt_901_asOf902_s778_26-0915.xlsx`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/ags_Yr_DbtChangesRslt_901_asOf902_s778_26-0915.xlsx), quality [`stage2_qi_quality_902_26-0915.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/stage2_qi_quality_902_26-0915.json), groups [`stage2_qi_discrepancy_groups_902_26-0915.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/stage2_qi_discrepancy_groups_902_26-0915.json).

**Остаточный defect (не блокер ∑/Split):** [`E-RowGrain-28469`](../../sql/26-0831-sudz-dbt-slot-link/stage2_qiv_excel_error_registry.md) — трек **S77.9**.

###### S77.9 — непрерывность слотов / E-RowGrain-28469 (2026-09-15)

**Контекст:** Access `26-0505` — **одна** строка СФ `28469` / КС-51 / 2 733 213 421,80. FEMSQ — два Dbt на одном `iKey=6760`: **3398** `idNum=1` (`ciaName=первая`, история …→902) и **3399** `idNum=2` (`ciaName=вторая`, Value@**901**). Rslt asOf902 «разъезжается» по полосам QIV/QI.

**Диагноз:** cutover (два слота на `первая`/`вторая`) **в целом верный** — оба зерна с фактами сохраняем (иначе ломается ∑ upl, где были два долга). Ошибка — **выбор слота воронкой** при однострочном последующем своде (цеплять к континууму, не к «чужому» слоту). Не путать с коллизией `unnamed`/`ciaName='1'`.

**Запреты:** не гасить слот «вторая» из‑за крошек; не auto-merge всех `первая`+`вторая` при seed; не full apply @902 ради лога.

###### Мероприятия S77.9 (порядок)

| # | Мероприятие | Содержание | Критерий | Статус |
|---|-------------|------------|----------|--------|
| **M1** | Эталон Access (data) | UI **Долг (канон):** Merge **3399→3398**; Value@**901** на survivor (3398). Rslt `asOf=902` — **одна** строка QIV+QI как Access | softAmt / глаз: 1 строка; ∑ без регресса | ✅ **2026-09-15** Rslt 1× row62 dbt **3398**; арт. [`…m1_26-0915.xlsx`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/ags_Yr_DbtChangesRslt_901_asOf902_m1_26-0915.xlsx) |
| **M2** | Откат только Value@902 по долгу | После PASS M1: удалить `DbtValue` `dvUpl=902` на слотах пары (после Merge — на survivor); хвосты P1/КСДД по ключам при наличии. **Не** трогать Tbl@902, Value@901, прочие долги | у 28469 нет Value@902; @901 на survivor | ✅ **2026-09-15** удалён `dvKey=103758`; @901=`103761` на 3398 |
| **M3** | Код: непрерывность + dry-лог | `dbtValueLoad` (± выбор слота): 1 строка Excel + N слотов на `iKey` → слот с Value@base/prev той же суммы (±0.01); иначе dry-лог + не silent. 2 строки Excel в upl — оба слота | dry пишет блок «непрерывность» / кандидатов | ✅ **2026-09-15** `contRank` base→last; `continuityRows` + HTML «Непрерывность слотов»; test `SudzDbtUplDbtValueLoadLogTest` |
| **M4** | UAT браузер | После M1–M3: dry `dbtValueLoad`@902 → лог не пустой Skip-only; apply → Value@902 на survivor; Rslt снова = Access | лог + эталон | ✅ **2026-09-15** dry: 28469→3398 Value@base; apply tail=1 (`dvKey=103762`); Rslt 1× [`…m4_26-0915.xlsx`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/ags_Yr_DbtChangesRslt_901_asOf902_m4_26-0915.xlsx) |
| **M5** | Cutover контроль | Слоты `первая`/`вторая` не схлопывать; опц. post-seed отчёт «iKey с N>1» | инфо, без удаления зёрен | ✅ **2026-09-15** cutover §1.6; SQL [`99b_REPORT_multi_slot_iKeys.sql`](../../sql/26-0827-sudz-m2-seed/99b_REPORT_multi_slot_iKeys.sql); контроль [`m5_multi_slot_control_26-0915.json`](../../sql/26-0831-sudz-dbt-slot-link/artifacts/m5_multi_slot_control_26-0915.json) (6760: 3398/3399) |

**Связь:** реестр [`E-RowGrain-28469`](../../sql/26-0831-sudz-dbt-slot-link/stage2_qiv_excel_error_registry.md) (`fixed` data; код continuity = M3 ✅); UI Merge — **S78**; cutover seed слотов не переписывать под «один слот».

##### S78 — экран канона Dbt: поиск и связь слотов (2026-09-09)

**Задача:** 0069. **Чат:** `chat-2026-09-08-002`. **Инструкция:** [sudz-operator-guide.md](../../ops/sudz-operator-guide.md).

**Цель:** оператор находит любой канон и собирает произвольную конфигурацию слотов через **привязать / отвязать** (`invDbtDbt`). Split/Merge — производные сценарии (инструкция), не первичные кнопки MVP.

**Поиск (верх, во всю ширину):** № СФ → № договора → исполнитель / дата стороны → № слота в СФ (`idNum`) → `dbtKey`. Практика: фильтр по СФ (± договор) → десяток кандидатов → выбор. `dbtKey` делает открытие однозначным.

**Карточка (верх→низ):** таблица слотов → сплиттер **FemsqTree** (слева) / **FemsqChart** (справа). Корни дерева — слоты **этого** канона (не walk-tree СФ). Раскрытие: var → DbtValue (upl в заголовке). График: на слот две **области** (stack внутри слота): просрочено / непросрочено (`ttl−overd`); ось X — `uplStatusOnDate`. Не `RelationTree` `inv-dbt-slots` (там все слоты СФ, в т.ч. чужие Dbt). Не timeline КСДД (`ciudKey`).

**Команды:** привязать / отвязать слот; модалки **Value** (задать/снять — иначе unlink мёртв); **Split** / **Merge** — обёртки над `splitSudzDbt` / `mergeSudzDbt`, не «волшебные» кнопки без атомов.

**Не смешивать** с «Долги / мероприятия» (Rslt/cmm): отдельный пункт меню **Долг (канон)**.

| Шаг | Содержание | Статус |
|-----|------------|--------|
| **S78.1** | Инструкция-прототип + текст в плане | ✅ 2026-09-09 |
| **S78.2** | GraphQL search/detail + link/unlink | ✅ 2026-09-09 |
| **S78.3** | UI: FemsqTable сверху, карточка снизу | ✅ 2026-09-09 |
| **S78.4A** | feQuLib: `area` + `stack` у FemsqChart | ✅ 2026-09-09 |
| **S78.4B** | GraphQL: даты/имя upl на `SudzDbtCanonValue` | ✅ 2026-09-09 |
| **S78.4C** | Сплиттер: дерево (чтение) + график всех слотов | ✅ 2026-09-09 |
| **S78.4D** | GraphQL + модалка upsert/delete Value | ✅ 2026-09-09 |
| **S78.4E** | Модалки Split / Merge на каноне | ✅ 2026-09-09 |
| **S78.6** | GraphQL + вкладка Комментарии: load/upsert/delete по `dvKey` | ✅ 2026-09-09 |
| **S78.5** | Связки из КСДД / Rslt на карточку | ☐ later |

Параллельно: **S77.7b** (dry-лог / слоты @902) → gate'ы 902 → **S77.8**; UI-UAT L*/Split на S78/КСДД — **после** рамочной сверки.

##### S79 — двушаговый контур ложных погашений перед Rslt сбор (2026-09-16)

**Задача:** **0075**. **Процесс:** [03 §1.1.1.3](../../domain/sudz/03-processes.md) · UI [02-9 Progress](../../UI/02-9_sudz-mvp-screens.md) · домен P1/`overdReduced` [04-6](../../domain/sudz/04-6_multi-dbt-p1-three-paths.md).  
...
**Рабочий план UI:** [chat-plan-26-0819-cn-inv-pmt-upl.md](./chat-plan-26-0819-cn-inv-pmt-upl.md) · **0072–0074** ✅ · **0076** ✅ (канон apply/`g_p`/Rslt, H7 FAIL=0 2026-09-20) · эскиз [02-9 §4b](../../UI/02-9_sudz-mvp-screens.md#4b-экран-d--загрузка-платежей-cn_inv_pm_upl--лаунчер-file_f) · резюме v1 [chat-resume-26-0819](../chat-resume/chat-resume-26-0819-cn-inv-pmt-upl.md).

**Решение владельца (2026-08-19):** visual analog экрана C без пресетов; лог на вкладке; нижние вкладки ход / повторы СФ / стройки новые; список = все `cn_inv_pm_upl`; вкладка «выгрузки платежей» на C = мост `g_p` (1 дбт → N pmt), не этот экран. Visual v1 принят 2026-08-19.

**Статус (2026-09-20):** план 0819 / задача **0076** **закрыты** (§0a.1–5). Later вне этого чата: вкладка g_p на C, КСДСФ pmt. UI-детали — только в 26-0819.
