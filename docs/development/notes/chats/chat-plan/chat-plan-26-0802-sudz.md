...
**Проект:** FEMSQ  
**Версия плана:** 0.99.24 (S77.3 GraphQL Split/Merge; C.10.3/C.10.5 — блокер S77)
**Задача:** 0065–0072 (дерево features **02.03**); эскизы [02-9](../../UI/02-9_sudz-mvp-screens.md); **активно: 0069** — **S77** split/merge (инварианты→DDL→ядро→воронка→Rslt A/C); **S76-C.10** ⛔ до S77; **S74 M5**; **0071** 🔶; **S69**/**S70** ✅
**Статус плана:** ✅ 0070; **S75** stage1 ✅; **S76** 🔄 (**C.10.1 G4** фикстура 85166); **S77** 🔄 **S77.1–.3** ✅; next **S77.4** КСДД Split-советник; **S74 M1–M4** ✅; **M5 C1** ✅; **0071** 🔶
...
| S77 | 2026-09-08 | **split/merge в живом `sudz` (DEV)** и воронке: инварианты → DDL → Split/Merge → КСДД → экспортёр A/C → UAT 85166 → 901/902 → `26-0505`. Не узкий срез. | [§5.6 S77](#s77--splitmerge-ядро--воронка--rslt-ac-dev-2026-09-08); черновик [26-0908-sudz-split-merge-dev](../../sql/26-0908-sudz-split-merge-dev/) | 🔄 S77.4; GraphQL Split/Merge ✅ |
...
**Следующий шаг в чате:** **S77.4** — советник КСДД «свод N×S vs ∑ на слоте → Split, не Link». S77.3 ✅ (`splitSudzDbt` / `mergeSudzDbt`). S77.1 ✅. S77.2 ✅ (`01` на DEV `sudz`). **C.10.1 G4** @901 (ciud 2807/2808, `iKey=85166`) остаётся **фикстурой UAT** S77. **C.10.3 / C.10.5** — **блокер S77**. Канон полос A/C: [08](../../sql/26-0908-sudz-split-merge-sandbox/08_CMM_AND_RSLT.md). Кнопки КСДД и экспортёр **не** трогались.
...
| **C.10.1** | UI **901**: excelToTbl → фазы A–E (G0…G5) | G2–G4 open=0; DV≈Tbl (±док. искл.) | 🔶 G3@901 open=0; G4 open=**2** (2807/2808, 85166) — **фикстура S77**; DV=12 / Tbl=1774; Create/Link без split нельзя |
...
| **S77.2** | DDL DEV (`sudz` only): DROP UX + переписать Consistency п.4, убрать п.5 | ✅ DEV 2026-09-08; `test_sudz` не тронут; `02` *Dv не применялся |
| **S77.3** | Мутации GraphQL **Split** / **Merge** (те же, что вызовут экран и воронка) | ✅ 2026-09-08; UI КСДД не вешался |
| **S77.4** | КСДД / `invDbtLoad`: советник «свод N×S, на слоте ∑=N×S → Split, не Link»; после Split `dbtValueLoad` пишет N строк | ☐ |
| **S77.5** | Экспортёр полос **A/C** (`SudzRsltExcelExporter` сейчас 1 строка на `Dbt`) | ☐ |
| **S77.6** | UAT 85166 @901: Split → 2×18000 → G4 `open=0` | ☐ фикстура 2807/2808 |
...
**GraphQL (S77.3):** `splitSudzDbt(input: SplitSudzDbtInput!)` / `mergeSudzDbt(input: MergeSudzDbtInput!)` (`CANONS` | `SHARES_ON_UPL`). Java: `JdbcSudzDao.splitDbt` / `mergeDbt`; Apollo: `splitSudzDbt` / `mergeSudzDbt` в `sudz-api.ts`. Split **не** идёт через очередь Create/Link и **не** вызывает `ensureInvDbtDbtBridgeForSlot` (тот helper завёл бы новый `Dbt`). Кнопки КСДД — S77.4.
