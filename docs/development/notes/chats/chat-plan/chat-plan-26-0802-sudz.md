...
**Проект:** FEMSQ  
**Версия плана:** 0.97.1 (вкладка «двоящие долги СФ» на лаунчере)  
**Задача:** 0065–0072 (дерево features **02.03**); эскизы [02-9](../../UI/02-9_sudz-mvp-screens.md); **активно: 0069** — next **UAT ensure/load upl 910**; **0071** Договоры 🔶; **S69**/**S70** ✅  
**Статус плана:** ✅ 0070; **S68u** коллизии 910 ✅; **S66** Inv 693 ✅; **S66a AccSmpl 705** ✅; **S66b** сверка cidu ✅; **S66c** целевая воронка ✅; **S66e** порядок 1–17 ✅ + **код v1** ensure/load/очередь + вкладка лаунчера; **0071** 🔶; **S69**/**S70** ✅  
...
| S66e | 2026-08-24 | **Порядок `invDbt`** сегм. **1–17** ✅; владелец: порядок достаточен → **код v1:** `invDbtVarEnsure`/`invDbtLoad`, DDL `CnInvUplInvDbtDouble`, stub экрана; Access-хвост disabled; `Dbt`/`DbtValue` отложены | [§5.6 S66e](#s66e--порядок-определения-invdbt-сегментами-2026-08-24); [sql/26-0824](../../sql/26-0824-sudz-inv-dbt-double/) | 🔶 UAT 910 |
...
| 15 | 2026-08-24 | **`Dbt`/`DbtValue`:** отложены; этап = `invDbtVarEnsure` + `invDbtLoad` (+ очередь) | ✅ |
...
**Сегмент 15 — Dbt / DbtValue отложены (владелец, 2026-08-24):**

Вариант **1:** этап после AccSmpl пишет только **`invDbtVar` / `invDbt` / `invDbtDbtVar`** (+ очередь разбора). **`Dbt`**, **`invDbtDbt`**, **`DbtValue`** — отдельный следующий кусок, не смешивать с `invDbtLoad`. Критерий 0069 на Dbt/DbtValue остаётся, но не блокирует старт слота/варианта.
...
| `invDbtLoad` | **новый** (S66e) | всегда rebuild `CnInvUplInvDbtDouble`; при `flLoad` — auto `invDbt`+`invDbtDbtVar` (calm); экран stub `sudz-inv-dbt-double` |
...
5. После AccSmpl apply только в целевой контур `sudz.invDbt*` (**S66c/S66e**); не в `cnInvAccnt` / `cn_inv_dbt`. `Dbt`/`DbtValue` — отдельный этап (сегм. 15).

Итерации разработки: … → AccSmpl ✅ → S66e порядок ✅ → **код ensure/load v1** ← сейчас (**UAT 910**) → экран разбора → `Dbt`/`DbtValue` → регрессия Rslt.  
