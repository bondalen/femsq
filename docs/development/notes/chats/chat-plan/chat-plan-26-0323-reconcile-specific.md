...
## Подготовка к продуктиву (без кода)

> Цель блока: безопасно перейти от подтверждённого dev/stage сценария к проверке в браузере на машине разработки и затем к переносу на продуктивную машину.
> Ограничение объёма: функционал как при предыдущем развёртывании + загрузка Excel для type 5.

### P.1. Scope freeze (рамки функционала)
- ✅ P.1.1. Зафиксирован baseline функционала предыдущего развёртывания (браузерные разделы и сценарии) по `chat-resume-26-0305.md` и `chat-resume-26-0310-audit-execution-infra.md`.
- ✅ P.1.2. Зафиксировано допустимое расширение: только Type 5 Excel (staging/reconcile/audit-log), без включения новых доменных сценариев Type 2/3/6.
...
- [ ] P.3.2. Выполнить сценарий Type 5 в UI: запуск ревизии, рост `adt_results`, завершение `COMPLETED`.
- [ ] P.3.3. Подтвердить инфраструктурные инварианты: async-запуск, корректный polling, отсутствие “залипания” кнопки запуска.

### P.4. DB safety и rollback readiness
- [ ] P.4.1. Подготовить и проверить backup/restore план для целевой БД/схемы перед продуктивным окном.
- [ ] P.4.2. Зафиксировать критерии немедленного отката (триггеры rollback) при продуктивной проверке.
- ✅ P.4.3. Подтверждена операционная доступность post-apply smoke-check и marker policy (`README-TESTING`, SQL-плейбуки).

### P.5. Обязательный post-apply контроль
...
- [ ] P.6.2. NO-GO: есть незакрытые критичные дефекты UI/выполнения ревизии, не подтверждён rollback, либо smoke-check не PASS.
- [ ] P.6.3. Зафиксировать финальное решение GO/NO-GO с датой, ответственным и ссылкой на отчёты проверки.
