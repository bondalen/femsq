...
> **Среда прогонов:** FishEye dev, `SQL_HOST=10.7.0.3`, цепь **5**, `@MounthEndDate=2022-12-31`.

- [x] **18.7.1** `FIXTURE_05` + `07n_plan_strict_cst_chain5.sql` (cstAgPn=**2102**, stIpg=**61**) ✅ **2026-06-17**
  - **Артефакты:** `FIXTURE_05_pilot_cst_2102.sql`, `07n_plan_strict_cst_chain5.sql`
  - **CORRECT:** **PASS**; fail timeline/yearend/revn/mst/add = **0** (К-12b/c/r, MASTERING @dt, К-13b)
  - **PERF:** ~**578 ms**
  - **Ход:** на prod-подобных данных UtPl привязан к **снятым** ИП 6/8; FIXTURE_05 добавил помесячный UtPl на актуальную ipgp **5271** (40%/60% в mn 9/11); `07n` подтвердил строгую логику накопленного плана в пределах ipgPn

- [x] **18.7.2a** `FIXTURE_06`: новые `ipgUtPlGr` + полный UtPl для **2037/3290/5271** (cst 2102); откат ✅ **2026-06-24**
  - **Артефакты:** `FIXTURE_06_00`→`06_01`→`06_golden`→`06_verify`, `apply_fixture_06.sh`; коммит `b09d116`
  - **CORRECT:** `06_verify` **PASS**
  - **Ход:** замена пилота FIXTURE_05; изолированные `ipgUtPlGr` **18–20** (swap `ipgcrvUtPlGr` 6/8/11 → 18/19/20); полный golden UtPl на трёх пунктах ИП одной стройки

- [x] **18.7.2b** `07o` / расширение 07n: матрица **17 дат**, критерии К-14…К-17, К-12t ✅ **2026-06-24** (cst 2102)
  - **Артефакты:** `07o_plan_17dates_cst_chain5.sql`, `run_07o_cst2102_17dates.sh`
  - **CORRECT:** **PASS** (К-14…К-17, К-12t на `fnIpgChDatsV(5)`)
  - **Ход:** матрица контрольных дат цепи; cum на 31.12 и на `ipgcrvEnd`; план только **активной** ИП на каждой дате

- [x] **18.7.2c** 5–10 пилотных строек с тремя пунктами ИП ✅ **2026-06-24** (**9 cst**, 07n+07o)
  - **Артефакты:** `FIXTURE_06_pilots_cst_chain5.sql`, `run_07n_o_pilots_chain5.sh`; коммит `83c4cf9`
  - **CORRECT:** **PASS 9/9** (2102, 121, 631, 1251, 1608, 1713, 2080, 2146, 2212)
  - **PERF:** ~**19 с** (оркестратор fixture + 07n + 07o по списку)
  - **Ход:** расширение golden-схемы FIXTURE_06 на восемь дополнительных cst; каждая стройка — три пункта ИП в цепи

- [x] **18.7.3** Агрегация: cstAgPn → stIpg → филиал → агент на **17 датах** ✅ **2026-06-24** (`07p`)
  - **Артефакты:** `07p_plan_aggregate_chain5.sql`, `run_07p_aggregate_chain5.sh`; коммит `f154aef`
  - **CORRECT:** **PASS**; К-18a–d; fail a/b/c/d = **0/0/0/0**; 27 golden ipgpPn, 9 пилотных cst
  - **PERF:** ~**504 ms**
...
#### Готовность к исполнению (2026-06-25)
...
| **К-13** | План: `212 = 172+187+195` @ `2022-12-31` (dev, fixture) | ✅ **2026-06-17** (07m) |
