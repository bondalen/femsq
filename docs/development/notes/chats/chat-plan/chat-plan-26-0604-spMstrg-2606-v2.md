...
**Дата:** 2026-06-04  
**Версия плана:** v2.6 от 2026-06-15 (этап 14 — К-7 строгая: ступени 1–3)  
**Предыдущая:** v2.5 от 2026-06-13 (этапы 11–12 ✅; **этап 13** — stCost 195 ✅ К-11)  
...
- [ ] **14.2.4** `MSSQL2012/03b1b` + полное зеркало `03c` (текущий MSSQL2012/03c устарел — resync из dev)
...
> Выполнять **только если** после 14.2 fn2 NULL **> 90 с**.

- [ ] **14.3.1** `04` → `spIpgChRsltCstUtl2_2606` (или inline в `06`): `#schemeRows` + `INDEX (ipgKey, ipgpCstAgPn, iShKey, mNum)`; `#mastMonthEnd`, `#branchCache` → `#temp`
- [ ] **14.3.2** `06` spMstrg_2606: вызов SP вместо `SELECT FROM fn2`; сохранить совместимость параметров для Access
- [ ] **14.3.3** `MSSQL2012/04` (или `06`), `08_ROLLBACK` — обновить
- [ ] **14.3.4** Приёмка: `07h6` К-7 **< 120 с**; `07f` PASS; `07_VERIFY_after`; `07h` stIpg=46/61

### Ступень 4 — Документация и деплой ⬜

- [ ] **14.4.1** `db-upgrade-spMstrg-2606.md` + deploy checklist — шаги 14.1–14.3
...
| Ступень 1+2 ✅ | **111,5 с** | ✅ |
| Ступень 3 | опционально | запас ~8 с |

---

## Следующий шаг реализации

**Ступень 3 (14.3)** — опционально: fn2→SP для дополнительного запаса; **К-7 уже достигнута** без неё.
