...
|---|-----------|-------------|--------|---------------|
| **U1** | Нет чёткого разделения светлой и тёмной темы лога; переключение неочевидно. Ориентир: **Kimbie Dark** (тёмная) и **Светлая (Visual Studio)** как в Cursor AI | **blocker** | ⏳ | 0049 → `audit-log.scss`, `AuditsView.vue` |
| **U2** | Межстрочный интервал в `adt_results` слишком большой (~в 3 раза больше нужного) | **blocker** | ⏳ | 0049 → `AuditExecutionContext.java` CSS / frontend |
| **U3** | Сообщение «пропущено — недостаточно обязательных данных» не информативно; нужны **конкретные поля** (первые + «и ещё N») | **blocker** | ⏳ | 0049 → `DefaultAuditStagingService.java` |
| **U4** | Строки Type5 match/apply на английском (`NEW`, `CHANGED`, `Type5 match — RA:…`) | **major** | ⏳ | 0049 → `AuditReconcileCoordinator.java`, `localizeMessageHtml()` |
| **U5** | Badge фазы **START** в логе — заменить на **`+`** | **minor** | ⏳ | 0049 → `AuditExecutionContext.java` |
...
**Решения оператора (2026-07-10):** тема по умолчанию — **тёмная (Kimbie Dark)**; для пропуска строк — **только первые обязательные поля + «и ещё N»**; знак фазы — **`+`** (согласовано с разработчиком).
...
##### 9.3.4.1. Темы лога (U1)

| # | Пункт | Статус |
|---|-------|--------|
| 9.3.4.1.1 | Вынести CSS `.femsq-auditlog` из `AuditExecutionContext.STYLE_BLOCK` во frontend (`audit-log.scss` или scoped в `AuditsView`) | ⏳ |
| 9.3.4.1.2 | Палитра **`.theme-kimbie-dark`** — тёплые тёмные тона (ориентир Cursor Kimbie Dark); **по умолчанию** | ⏳ |
| 9.3.4.1.3 | Палитра **`.theme-vs-light`** — светлый фон, тёмный текст (ориентир Cursor «Светлая (Visual Studio)»); контраст warning/error/summary в обеих темах | ⏳ |
| 9.3.4.1.4 | Переключатель в `AuditsView` (иконка/селект «Тема лога»); сохранение в `localStorage` (`femsq.auditLogTheme`) | ⏳ |
| 9.3.4.1.5 | Класс темы на контейнере `v-html` с `adt_results`; backend не хранит предпочтение темы | ⏳ |
...
| # | Пункт | Статус |
|---|-------|--------|
| 9.3.4.6.1 | Пересборка JAR **0.1.0.123+** с U6, U7, demote-siblings, 0049 backend-частью | ⏳ |
| 9.3.4.6.2 | Frontend: деплой с `files.ts` fix (U6) | ⏳ |
| 9.3.4.6.3 | Баннер директории (U8) — после 0049 или параллельно | ⏳ |

**Критерий приёмки 0049:** оператор на exec **1162** или новом dry-run подтверждает: тема по умолчанию тёмная, переключение на светлую работает; плотность строк приемлема; пропуски Excel с именами полей; Type5 на русском; badge `+`. Документация и реестр **9.3.3** обновлены.
...
| # | Пункт | Статус |
|---|-------|--------|
| 9.4.1 | Приоритизация: **blocker U1–U3, U6** → **0049** → major (U4, U7) → minor (U5, U8, U9) | ⏳ |
| 9.4.2 | Реализация **0049** (§9.3.4); unit/smoke после каждого подпункта | ⏳ |
| 9.4.3 | Пересборка и деплой **0.1.0.123+** на dev (`:8080` + frontend) | ⏳ |
| 9.4.4 | Повторный просмотр лога на exec **1162–1166** (без нового apply) + сценарии **9.3.1** (type=5) | ⏳ |
| 9.4.5 | Закрытие таблицы замечаний **9.3.3** (все blocker = ✅) | ⏳ |
...
**Критерий завершения фазы 9:** perf-задачи 0046–0047 выполнены или явно отложены с обоснованием; **0049** закрыта (читаемость лога); UAT 9.3 пройден **без открытых blocker** в реестре 9.3.3; prod обновлён thin JAR; ревизии type=5 и type=3 работают через веб-интерфейс.

**Задачи в `project-development.json`:** 0046 (saveProgress), 0047 (batch INSERT + apply-smoke), **0049** (читаемость лога / UI blocker), 0048 (dev/UAT/prod thin deploy).
...
## Доп. факты (актуализировано 2026-07-10)

- **Фаза 9 (v0.10.0):** perf 0046–0047 → dev deploy → UAT UI type=3 ✅ (exec 1162–1166) → **0049** (читаемость лога, blocker U1–U5) → повторный sign-off → UAT type=5 → prod thin JAR. Задачи: `0046`, `0047`, `0049`, `0048`.

- **UAT RALP UI (2026-07-10):** март dry **1162**; июль dry **1163**; apply **1164** (~3m29s); откат **1165**; идемпотентность март **1166**. Домен после UAT: **420/408**, снимок **2026_03**.

- **Blocker читаемости лога (2026-07-10):** оператор не воспринимает информацию в `adt_results` — зафиксированы U1–U5; решения: тема по умолчанию **Kimbie Dark**, пропуски — **первые 3 поля + «и ещё N»**, badge **`+`**.

- **База RALP dev (2026-07-09):** март `2026_03` = **420** valid (424 в Excel); июль `2026-07` = **1248** valid (+838). Домен trim до 420; staging март exec **1152**, июль exec **1153**.
