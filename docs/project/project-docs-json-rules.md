...
### Гибкая иерархия модулей (Java/Spring)

#### Поддерживаемые варианты вложенности

**Модули могут содержать:**
- ✅ **Субмодули** (рекомендуется) - через поле `submodules`
- ✅ **Компоненты** (допустимо) - через поле `components` 
- ⚠️ **Классы** (не рекомендуется) - через поле `classes`

**Субмодули могут содержать:**
- ✅ **Компоненты** (рекомендуется) - через поле `components`
- ✅ **Классы** (допустимо) - через поле `classes`

**Компоненты могут содержать:**
- ✅ **Субкомпоненты** (рекомендуется) - через поле `subcomponents`
- ✅ **Классы** (рекомендуется) - через поле `classes`

**Субкомпоненты могут содержать:**
- ✅ **Классы** (рекомендуется) - через поле `classes`
- ✅ **Вложенные субкомпоненты** (допустимо) - через поле `subcomponents`

#### Рекомендуемые паттерны

**Паттерн 1: Строгая иерархия (Рекомендуется)**
```
Модуль → Субмодуль → Компонент → Субкомпонент → Класс
```

**Паттерн 2: Гибридная структура (Допустимо)**
```
Модуль → Компонент → Класс
Модуль → Субмодуль → Класс
```

**Паттерн 3: Многоуровневая вложенность (Допустимо)**
```
Модуль → Субмодуль → Компонент → Субкомпонент → Субкомпонент → Класс
```

#### Не рекомендуемые варианты

❌ **Избегать:**
- Прямое размещение классов в корне модуля
- Слишком глубокую вложенность (более 5 уровней)
- Смешивание разных типов артефактов на одном уровне

#### Соответствие технологиям Java/Spring

- **Модули** = Maven/Gradle modules (корневые проекты)
- **Субмодули** = Maven sub-modules (вложенные проекты)
- **Компоненты** = Java packages (com.example.package)
- **Субкомпоненты** = Nested Java packages (com.example.package.internal)
- **Классы** = Java classes/interfaces/enums

#### Нумерация для гибкой структуры

- **Модули:** 01, 02, 0a, 0b
- **Субмодули:** 01.01, 01.02, 01.0a
- **Компоненты:** 01.01.01, 01.01.02, 01.02.01
- **Субкомпоненты:** 01.01.01.01, 01.01.01.02
